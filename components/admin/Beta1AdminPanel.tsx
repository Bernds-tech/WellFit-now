  ...
  return (
    <section className="space-y-4 text-sm text-slate-100">
      ...
      <section className="rounded border border-white/20 bg-slate-900/50 p-3 text-xs text-white/85">
        ...
        <p className="mt-3 rounded border border-cyan-400/30 bg-cyan-300/10 p-2">Agent GitHub Runner: serverseitige Config erforderlich, keine Secrets im Client, kein Production Deploy, Auto-Merge nur bei gruenen Gates.</p>
        ...
        <AdminForm title="Create GitHub PR" fields={["jobId"]} feedback={feedback["github-runner-pr"]} onSubmit={async (fd)=>{await run("github-runner-pr",()=>beta1AdminClient.createAgentGithubPullRequest({jobId: normalize(fd.get("jobId"))}));}} />
        ...
        <AdminForm title="Execute GitHub Auto-Merge" fields={["jobId"]} feedback={feedback["github-runner-exec"]} onSubmit={async (fd)=>{await run("github-runner-exec",()=>beta1AdminClient.executeAgentGithubAutoMergeMetadataOrReal({jobId: normalize(fd.get("jobId"))}));}} />
