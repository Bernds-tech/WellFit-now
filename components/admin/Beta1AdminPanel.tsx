  ...
  return (
    <section className="space-y-4 text-sm text-slate-100">
      ...
      <section className="rounded border border-white/20 bg-slate-900/50 p-3 text-xs text-white/85">
        ...
        <p className="mt-3 rounded border border-cyan-400/30 bg-cyan-300/10 p-2">Agent GitHub Runner: echte PR/Merge-Calls nur mit serverseitiger GitHub-API-Implementierung. Ohne echte API bleiben Status metadata_only/missing_server_config/github_api_not_implemented. Keine Secrets im Client, kein Production Deploy.</p>
        <AdminForm title="Prepare GitHub Runner Job" fields={["workerQueueId","policyId"]} feedback={feedback["github-runner-prepare"]} onSubmit={async (fd)=>{await run("github-runner-prepare",()=>beta1AdminClient.prepareAgentGithubRunnerJob({workerQueueId: normalize(fd.get("workerQueueId")), policyId: normalize(fd.get("policyId"))}));}} />
        <AdminForm title="Create PR / Metadata" fields={["jobId","title","body","baseBranch"]} feedback={feedback["github-runner-pr"]} onSubmit={async (fd)=>{ const jobId = normalize(fd.get("jobId")); const title = normalize(fd.get("title")); const body = normalize(fd.get("body")); const baseBranch = normalizeOptional(fd.get("baseBranch")); if (!jobId || !title || !body) return setFeedback((prev) => ({ ...prev, ["github-runner-pr"]: { ...prev["github-runner-pr"], error: "jobId, title und body sind erforderlich.", success: "" } })); await run("github-runner-pr",()=>beta1AdminClient.createAgentGithubPullRequest({jobId,title,body,baseBranch: baseBranch || undefined}));}} />
        <AdminForm title="Refresh GitHub Checks" fields={["jobId"]} feedback={feedback["github-runner-check-refresh"]} onSubmit={async (fd)=>{await run("github-runner-check-refresh",()=>beta1AdminClient.refreshAgentGithubCheckStatus({jobId: normalize(fd.get("jobId"))}));}} />
        <AdminForm title="Approve GitHub Auto-Merge" fields={["jobId"]} feedback={feedback["github-runner-approve"]} onSubmit={async (fd)=>{await run("github-runner-approve",()=>beta1AdminClient.approveAgentGithubAutoMerge({jobId: normalize(fd.get("jobId"))}));}} />
        <AdminForm title="Execute Auto-Merge / Metadata" fields={["jobId"]} feedback={feedback["github-runner-exec"]} onSubmit={async (fd)=>{await run("github-runner-exec",()=>beta1AdminClient.executeAgentGithubAutoMergeMetadataOrReal({jobId: normalize(fd.get("jobId"))}));}} />
        <AdminForm title="Block GitHub Runner Job" fields={["jobId","reason"]} feedback={feedback["github-runner-block"]} onSubmit={async (fd)=>{await run("github-runner-block",()=>beta1AdminClient.blockAgentGithubRunnerJob({jobId: normalize(fd.get("jobId")), reason: normalize(fd.get("reason"))}));}} />
      </section>
