...
const checks = [
  ...
  [world.includes('switchRenderMode("static")'), "Reduced-motion, unsupported WebGL and renderer errors converge on the static surface-bound mode"],
  [world.includes('if (nextMode !== "webgl")'), "Leaving WebGL explicitly settles the animated state machine"],
  [world.includes("clearMotionFinish();\n      journeyRef.current = null"), "Static/hidden transitions cancel pending motion completion and discard unfinished journeys"],
  [world.includes("this.props.onError()"), "Canvas/GLTF error boundary notifies the controller to leave WebGL mode"],
  [world.includes('onError={handleRendererFailure}'), "Renderer failure is wired to static mode instead of leaving WebGL timers active"],
  [world.includes('renderMode === "webgl" ? (\n        <>\n          <RudiRouteGuide'), "Route guides are mounted only while the animated WebGL world is active"],
  [!world.includes("useGLTF.preload"), "Invisible mobile/reduced-motion visitors do not eagerly preload the Rudi GLB pack"],
  [!world.includes("rudi-run.animation.glb") && !world.includes("rudi-alert.animation.glb") && !world.includes("rudi-point.animation.glb") && !world.includes("rudi-jump.animation.glb") && !world.includes("rudi-sit.animation.glb"), "Active DOM-world controller fetches only animation clips it can actually select"],
  [world.includes('image.style.opacity = "0"'), "Static fallback stays hidden until a real DOM anchor exists"],
