...
const ALLOWED_POLICY_ACTIVATION_STATES = new Set(["report_only", "single_agent_docs_execution", "single_agent_docs_register_build"]);
...
function main() {
  ...
  const nonMutatingDryRun = ALLOWED_POLICY_ACTIVATION_STATES.has(policy.activationState) && state.activationState === "report_only";
  const docsExecutionActivated = ["single_agent_docs_execution", "single_agent_docs_register_build"].includes(policy.activationState);
