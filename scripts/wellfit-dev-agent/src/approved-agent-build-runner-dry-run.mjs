...
function domainRiskFor(entry) { return entry ? String(entry.domainRisk ?? entry.riskLevel ?? "unknown") : "none"; }
function implementationRiskFor(entry) {
  return entry ? String(entry.implementationRisk ?? "blocked_until_separate_human_approval_names_exact_runtime_files_boundaries_and_tests") : "none";
}
...
function main() {
  ...
  const domainRisk = domainRiskFor(selected);
  const implementationRisk = implementationRiskFor(selected);
  const runtimeAuthorityGranted = false;
  ...
  console.log(`domainRisk: ${domainRisk}`);
  console.log(`implementationRisk: ${implementationRisk}`);
  console.log(`runtimeAuthorityGranted: ${runtimeAuthorityGranted}`);
  console.log(`reportOnly: ${reportOnly}`);
