...
const ALLOWED_RUNNER_ACTIVATION_STATES = ["framework_only", "single_agent_docs_register_build"];
...
function collectPolicyChecks(policy, batchExecutionPolicy, batchPolicy, taskQueue, autoMergePolicy, autoRepairPolicy, prPostCreationGuard) {
  ...
  add("Runner activation state is framework_only or approved single-agent docs/register build", ALLOWED_RUNNER_ACTIVATION_STATES.includes(policy.activationState), policy.activationState ?? "missing");
...
function main() {
  ...
  console.log(`Docs/register/check-script execution may be activated: ${policy.activationState === "single_agent_docs_register_build"}`);
