...
const agentSteps = [
  ...
  { label: "Human Motivation Engine check", script: "scripts/wellfit-dev-agent/src/human-motivation-engine-check.mjs", displayCommand: "node scripts/wellfit-dev-agent/src/human-motivation-engine-check.mjs" },
  { label: "Ethical Engagement Guard check", script: "scripts/wellfit-dev-agent/src/ethical-engagement-guard-check.mjs", displayCommand: "node scripts/wellfit-dev-agent/src/ethical-engagement-guard-check.mjs" },
  { label: "Adaptive Difficulty Agent check", script: "scripts/wellfit-dev-agent/src/adaptive-difficulty-agent-check.mjs", displayCommand: "node scripts/wellfit-dev-agent/src/adaptive-difficulty-agent-check.mjs" },
  { label: "Multisensory Learning Engine check", script: "scripts/wellfit-dev-agent/src/multisensory-learning-engine-check.mjs", displayCommand: "node scripts/wellfit-dev-agent/src/multisensory-learning-engine-check.mjs" },
  { label: "Product Intelligence Agent check", script: "scripts/wellfit-dev-agent/src/product-intelligence-agent-check.mjs", displayCommand: "node scripts/wellfit-dev-agent/src/product-intelligence-agent-check.mjs" },
  { label: "Mission Factory Agent check", script: "scripts/wellfit-dev-agent/src/mission-factory-agent-check.mjs", displayCommand: "node scripts/wellfit-dev-agent/src/mission-factory-agent-check.mjs" },
  { label: "Healthy Retention Agent check", script: "scripts/wellfit-dev-agent/src/healthy-retention-agent-check.mjs", displayCommand: "node scripts/wellfit-dev-agent/src/healthy-retention-agent-check.mjs" },
  { label: "Approved Agent Build Runner check", script: "scripts/wellfit-dev-agent/src/approved-agent-build-runner-check.mjs", displayCommand: "node scripts/wellfit-dev-agent/src/approved-agent-build-runner-check.mjs" },
  { label: "Approved Agent Build Runner dry run", script: "scripts/wellfit-dev-agent/src/approved-agent-build-runner-dry-run.mjs", displayCommand: "node scripts/wellfit-dev-agent/src/approved-agent-build-runner-dry-run.mjs" },
  { label: "Approved Agent Build Runner merge gate check", script: "scripts/wellfit-dev-agent/src/approved-agent-build-runner-merge-gate-check.mjs", displayCommand: "node scripts/wellfit-dev-agent/src/approved-agent-build-runner-merge-gate-check.mjs" }
];
...
function main() {
  ...
  const humanMotivationEngineReport = readTextSafe("scripts/wellfit-dev-agent/output/human-motivation-engine-report.md");
  const ethicalEngagementGuardReport = readTextSafe("scripts/wellfit-dev-agent/output/ethical-engagement-guard-report.md");
  const adaptiveDifficultyAgentReport = readTextSafe("scripts/wellfit-dev-agent/output/adaptive-difficulty-agent-report.md");
  const multisensoryLearningEngineReport = readTextSafe("scripts/wellfit-dev-agent/output/multisensory-learning-engine-report.md");
  const productIntelligenceAgentReport = readTextSafe("scripts/wellfit-dev-agent/output/product-intelligence-agent-report.md");
  const missionFactoryAgentReport = readTextSafe("scripts/wellfit-dev-agent/output/mission-factory-agent-report.md");
  const healthyRetentionAgentReport = readTextSafe("scripts/wellfit-dev-agent/output/healthy-retention-agent-report.md");
  const approvedAgentBuildRunnerReport = readTextSafe("scripts/wellfit-dev-agent/output/approved-agent-build-runner-check-report.md");
  const approvedAgentBuildRunnerDryRunReport = readTextSafe("scripts/wellfit-dev-agent/output/approved-agent-build-runner-dry-run-report.md");
  const approvedAgentBuildRunnerMergeGateReport = readTextSafe("scripts/wellfit-dev-agent/output/approved-agent-build-runner-merge-gate-check-report.md");
  ...
  const humanMotivationEngineStep = getStep(steps, "Human Motivation Engine check");
  const ethicalEngagementGuardStep = getStep(steps, "Ethical Engagement Guard check");
  const adaptiveDifficultyAgentStep = getStep(steps, "Adaptive Difficulty Agent check");
  const multisensoryLearningEngineStep = getStep(steps, "Multisensory Learning Engine check");
  const productIntelligenceAgentStep = getStep(steps, "Product Intelligence Agent check");
  const missionFactoryAgentStep = getStep(steps, "Mission Factory Agent check");
  const healthyRetentionAgentStep = getStep(steps, "Healthy Retention Agent check");
  const approvedAgentBuildRunnerStep = getStep(steps, "Approved Agent Build Runner check");
  const approvedAgentBuildRunnerDryRunStep = getStep(steps, "Approved Agent Build Runner dry run");
  const approvedAgentBuildRunnerMergeGateStep = getStep(steps, "Approved Agent Build Runner merge gate check");
  ...
  const autoRepairDecisionReportOnly = /Mode:\s*(?:REPORT_ONLY|SAFE_DOCS_REGISTER_JSON_FORMAT_REPAIR_ALLOWED)/i.test(`${autoRepairDecisionReport}\n${autoRepairDecisionStep?.stdout ?? ""}`) && /Never merges:\s*true/i.test(`${autoRepairDecisionReport}\n${autoRepairDecisionStep?.stdout ?? ""}`) && /AUTO_REPAIR_ALLOWED=(?:true|false)/i.test(`${autoRepairDecisionReport}\n${autoRepairDecisionStep?.stdout ?? ""}`) && /safe docs\/register\/JSON-format repair/i.test(`${autoRepairDecisionReport}\n${autoRepairDecisionStep?.stdout ?? ""}`);
  ...
  const humanMotivationEngineReady = /Mode:\s*REPORT_ONLY/i.test(`${humanMotivationEngineReport}\n${humanMotivationEngineStep?.stdout ?? ""}`) && /Never modifies runtime files:\s*true/i.test(`${humanMotivationEngineReport}\n${humanMotivationEngineStep?.stdout ?? ""}`) && /Never personalizes users at runtime:\s*true/i.test(`${humanMotivationEngineReport}\n${humanMotivationEngineStep?.stdout ?? ""}`) && /Never approves PRs:\s*true/i.test(`${humanMotivationEngineReport}\n${humanMotivationEngineStep?.stdout ?? ""}`) && /Never merges PRs:\s*true/i.test(`${humanMotivationEngineReport}\n${humanMotivationEngineStep?.stdout ?? ""}`) && /Never repairs files:\s*true/i.test(`${humanMotivationEngineReport}\n${humanMotivationEngineStep?.stdout ?? ""}`) && /Never deploys:\s*true/i.test(`${humanMotivationEngineReport}\n${humanMotivationEngineStep?.stdout ?? ""}`) && /HUMAN_MOTIVATION_ENGINE_READY=true/i.test(`${humanMotivationEngineReport}\n${humanMotivationEngineStep?.stdout ?? ""}`);
  const ethicalEngagementGuardReady = /Mode:\s*REPORT_ONLY/i.test(`${ethicalEngagementGuardReport}\n${ethicalEngagementGuardStep?.stdout ?? ""}`) && /Never modifies runtime files:\s*true/i.test(`${ethicalEngagementGuardReport}\n${ethicalEngagementGuardStep?.stdout ?? ""}`) && /Never personalizes users at runtime:\s*true/i.test(`${ethicalEngagementGuardReport}\n${ethicalEngagementGuardStep?.stdout ?? ""}`) && /Never tracks or profiles users at runtime:\s*true/i.test(`${ethicalEngagementGuardReport}\n${ethicalEngagementGuardStep?.stdout ?? ""}`) && /Never approves PRs:\s*true/i.test(`${ethicalEngagementGuardReport}\n${ethicalEngagementGuardStep?.stdout ?? ""}`) && /Never merges PRs:\s*true/i.test(`${ethicalEngagementGuardReport}\n${ethicalEngagementGuardStep?.stdout ?? ""}`) && /Never repairs files:\s*true/i.test(`${ethicalEngagementGuardReport}\n${ethicalEngagementGuardStep?.stdout ?? ""}`) && /Never deploys:\s*true/i.test(`${ethicalEngagementGuardReport}\n${ethicalEngagementGuardStep?.stdout ?? ""}`) && /ETHICAL_ENGAGEMENT_GUARD_READY=true/i.test(`${ethicalEngagementGuardReport}\n${ethicalEngagementGuardStep?.stdout ?? ""}`);
  const adaptiveDifficultyAgentReady = /Mode:\s*REPORT_ONLY/i.test(`${adaptiveDifficultyAgentReport}\n${adaptiveDifficultyAgentStep?.stdout ?? ""}`) && /Never modifies runtime files:\s*true/i.test(`${adaptiveDifficultyAgentReport}\n${adaptiveDifficultyAgentStep?.stdout ?? ""}`) && /Never personalizes users at runtime:\s*true/i.test(`${adaptiveDifficultyAgentReport}\n${adaptiveDifficultyAgentStep?.stdout ?? ""}`) && /Never tunes difficulty at runtime:\s*true/i.test(`${adaptiveDifficultyAgentReport}\n${adaptiveDifficultyAgentStep?.stdout ?? ""}`) && /Never profiles protected data:\s*true/i.test(`${adaptiveDifficultyAgentReport}\n${adaptiveDifficultyAgentStep?.stdout ?? ""}`) && /Never enables reward authority:\s*true/i.test(`${adaptiveDifficultyAgentReport}\n${adaptiveDifficultyAgentStep?.stdout ?? ""}`) && /Never enables mission-completion authority:\s*true/i.test(`${adaptiveDifficultyAgentReport}\n${adaptiveDifficultyAgentStep?.stdout ?? ""}`) && /Never approves PRs:\s*true/i.test(`${adaptiveDifficultyAgentReport}\n${adaptiveDifficultyAgentStep?.stdout ?? ""}`) && /Never merges PRs:\s*true/i.test(`${adaptiveDifficultyAgentReport}\n${adaptiveDifficultyAgentStep?.stdout ?? ""}`) && /Never repairs files:\s*true/i.test(`${adaptiveDifficultyAgentReport}\n${adaptiveDifficultyAgentStep?.stdout ?? ""}`) && /Never deploys:\s*true/i.test(`${adaptiveDifficultyAgentReport}\n${adaptiveDifficultyAgentStep?.stdout ?? ""}`) && /ADAPTIVE_DIFFICULTY_AGENT_READY=true/i.test(`${adaptiveDifficultyAgentReport}\n${adaptiveDifficultyAgentStep?.stdout ?? ""}`);
  const multisensoryLearningEngineReady = /Mode:\s*REPORT_ONLY/i.test(`${multisensoryLearningEngineReport}\n${multisensoryLearningEngineStep?.stdout ?? ""}`) && /Never modifies runtime files:\s*true/i.test(`${multisensoryLearningEngineReport}\n${multisensoryLearningEngineStep?.stdout ?? ""}`) && /Never personalizes users at runtime:\s*true/i.test(`${multisensoryLearningEngineReport}\n${multisensoryLearningEngineStep?.stdout ?? ""}`) && /Never tracks or profiles users at runtime:\s*true/i.test(`${multisensoryLearningEngineReport}\n${multisensoryLearningEngineStep?.stdout ?? ""}`) && /Never activates AR or Unity behavior:\s*true/i.test(`${multisensoryLearningEngineReport}\n${multisensoryLearningEngineStep?.stdout ?? ""}`) && /Never enables reward authority:\s*true/i.test(`${multisensoryLearningEngineReport}\n${multisensoryLearningEngineStep?.stdout ?? ""}`) && /Never enables mission-completion authority:\s*true/i.test(`${multisensoryLearningEngineReport}\n${multisensoryLearningEngineStep?.stdout ?? ""}`) && /Never approves PRs:\s*true/i.test(`${multisensoryLearningEngineReport}\n${multisensoryLearningEngineStep?.stdout ?? ""}`) && /Never merges PRs:\s*true/i.test(`${multisensoryLearningEngineReport}\n${multisensoryLearningEngineStep?.stdout ?? ""}`) && /Never repairs files:\s*true/i.test(`${multisensoryLearningEngineReport}\n${multisensoryLearningEngineStep?.stdout ?? ""}`) && /Never deploys:\s*true/i.test(`${multisensoryLearningEngineReport}\n${multisensoryLearningEngineStep?.stdout ?? ""}`) && /MULTISENSORY_LEARNING_ENGINE_READY=true/i.test(`${multisensoryLearningEngineReport}\n${multisensoryLearningEngineStep?.stdout ?? ""}`);
  const productIntelligenceAgentReady = /Mode:\s*REPORT_ONLY/i.test(`${productIntelligenceAgentReport}\n${productIntelligenceAgentStep?.stdout ?? ""}`) && /Never modifies runtime files:\s*true/i.test(`${productIntelligenceAgentReport}\n${productIntelligenceAgentStep?.stdout ?? ""}`) && /Never makes product decisions automatically:\s*true/i.test(`${productIntelligenceAgentReport}\n${productIntelligenceAgentStep?.stdout ?? ""}`) && /Never personalizes users at runtime:\s*true/i.test(`${productIntelligenceAgentReport}\n${productIntelligenceAgentStep?.stdout ?? ""}`) && /Never tracks or profiles users at runtime:\s*true/i.test(`${productIntelligenceAgentReport}\n${productIntelligenceAgentStep?.stdout ?? ""}`) && /Never enables reward authority:\s*true/i.test(`${productIntelligenceAgentReport}\n${productIntelligenceAgentStep?.stdout ?? ""}`) && /Never enables mission-completion authority:\s*true/i.test(`${productIntelligenceAgentReport}\n${productIntelligenceAgentStep?.stdout ?? ""}`) && /Never approves PRs:\s*true/i.test(`${productIntelligenceAgentReport}\n${productIntelligenceAgentStep?.stdout ?? ""}`) && /Never merges PRs:\s*true/i.test(`${productIntelligenceAgentReport}\n${productIntelligenceAgentStep?.stdout ?? ""}`) && /Never repairs files:\s*true/i.test(`${productIntelligenceAgentReport}\n${productIntelligenceAgentStep?.stdout ?? ""}`) && /Never deploys:\s*true/i.test(`${productIntelligenceAgentReport}\n${productIntelligenceAgentStep?.stdout ?? ""}`) && /PRODUCT_INTELLIGENCE_AGENT_READY=true/i.test(`${productIntelligenceAgentReport}\n${productIntelligenceAgentStep?.stdout ?? ""}`);
  const missionFactoryAgentReady = /Mode:\s*REPORT_ONLY/i.test(`${missionFactoryAgentReport}\n${missionFactoryAgentStep?.stdout ?? ""}`) && /Never modifies runtime files:\s*true/i.test(`${missionFactoryAgentReport}\n${missionFactoryAgentStep?.stdout ?? ""}`) && /Never creates runtime missions:\s*true/i.test(`${missionFactoryAgentReport}\n${missionFactoryAgentStep?.stdout ?? ""}`) && /Never authorizes mission completion:\s*true/i.test(`${missionFactoryAgentReport}\n${missionFactoryAgentStep?.stdout ?? ""}`) && /Never authorizes rewards:\s*true/i.test(`${missionFactoryAgentReport}\n${missionFactoryAgentStep?.stdout ?? ""}`) && /Never tracks or profiles users at runtime:\s*true/i.test(`${missionFactoryAgentReport}\n${missionFactoryAgentStep?.stdout ?? ""}`) && /Never activates location\/camera\/health\/child\/legal\/token\/wallet\/payment\/Unity behavior:\s*true/i.test(`${missionFactoryAgentReport}\n${missionFactoryAgentStep?.stdout ?? ""}`) && /Never approves PRs:\s*true/i.test(`${missionFactoryAgentReport}\n${missionFactoryAgentStep?.stdout ?? ""}`) && /Never merges PRs:\s*true/i.test(`${missionFactoryAgentReport}\n${missionFactoryAgentStep?.stdout ?? ""}`) && /Never repairs files:\s*true/i.test(`${missionFactoryAgentReport}\n${missionFactoryAgentStep?.stdout ?? ""}`) && /Never deploys:\s*true/i.test(`${missionFactoryAgentReport}\n${missionFactoryAgentStep?.stdout ?? ""}`) && /MISSION_FACTORY_AGENT_READY=true/i.test(`${missionFactoryAgentReport}\n${missionFactoryAgentStep?.stdout ?? ""}`);
  const healthyRetentionAgentReady = /Mode:\s*REPORT_ONLY/i.test(`${healthyRetentionAgentReport}\n${healthyRetentionAgentStep?.stdout ?? ""}`) && /Never modifies runtime files:\s*true/i.test(`${healthyRetentionAgentReport}\n${healthyRetentionAgentStep?.stdout ?? ""}`) && /Never personalizes users at runtime:\s*true/i.test(`${healthyRetentionAgentReport}\n${healthyRetentionAgentStep?.stdout ?? ""}`) && /Never tracks or profiles users at runtime:\s*true/i.test(`${healthyRetentionAgentReport}\n${healthyRetentionAgentStep?.stdout ?? ""}`) && /Never triggers notifications:\s*true/i.test(`${healthyRetentionAgentReport}\n${healthyRetentionAgentStep?.stdout ?? ""}`) && /Never modifies streaks or rewards:\s*true/i.test(`${healthyRetentionAgentReport}\n${healthyRetentionAgentStep?.stdout ?? ""}`) && /Never authorizes mission completion:\s*true/i.test(`${healthyRetentionAgentReport}\n${healthyRetentionAgentStep?.stdout ?? ""}`) && /Never authorizes rewards:\s*true/i.test(`${healthyRetentionAgentReport}\n${healthyRetentionAgentStep?.stdout ?? ""}`) && /Never activates location\/camera\/health\/child\/legal\/token\/wallet\/payment\/Unity behavior:\s*true/i.test(`${healthyRetentionAgentReport}\n${healthyRetentionAgentStep?.stdout ?? ""}`) && /Never approves PRs:\s*true/i.test(`${healthyRetentionAgentReport}\n${healthyRetentionAgentStep?.stdout ?? ""}`) && /Never merges PRs:\s*true/i.test(`${healthyRetentionAgentReport}\n${healthyRetentionAgentStep?.stdout ?? ""}`) && /Never repairs files:\s*true/i.test(`${healthyRetentionAgentReport}\n${healthyRetentionAgentStep?.stdout ?? ""}`) && /Never deploys:\s*true/i.test(`${healthyRetentionAgentReport}\n${healthyRetentionAgentStep?.stdout ?? ""}`) && /HEALTHY_RETENTION_AGENT_READY=true/i.test(`${healthyRetentionAgentReport}\n${healthyRetentionAgentStep?.stdout ?? ""}`);
  const approvedAgentBuildRunnerReady = /Mode:\s*REPORT_CHECK_ONLY/i.test(`${approvedAgentBuildRunnerReport}\n${approvedAgentBuildRunnerStep?.stdout ?? ""}`) && /Policy allows docs\/register\/validator agent builds:\s*true/i.test(`${approvedAgentBuildRunnerReport}\n${approvedAgentBuildRunnerStep?.stdout ?? ""}`) && /Never builds runtime agents:\s*true/i.test(`${approvedAgentBuildRunnerReport}\n${approvedAgentBuildRunnerStep?.stdout ?? ""}`) && /PR creation allowed for scoped docs\/register agent builds:\s*true/i.test(`${approvedAgentBuildRunnerReport}\n${approvedAgentBuildRunnerStep?.stdout ?? ""}`) && /Never merges PRs:\s*true/i.test(`${approvedAgentBuildRunnerReport}\n${approvedAgentBuildRunnerStep?.stdout ?? ""}`) && /Never repairs files:\s*true/i.test(`${approvedAgentBuildRunnerReport}\n${approvedAgentBuildRunnerStep?.stdout ?? ""}`) && /Never deploys:\s*true/i.test(`${approvedAgentBuildRunnerReport}\n${approvedAgentBuildRunnerStep?.stdout ?? ""}`) && /Missing checks are not merge-ready:\s*true/i.test(`${approvedAgentBuildRunnerReport}\n${approvedAgentBuildRunnerStep?.stdout ?? ""}`) && /APPROVED_AGENT_BUILD_RUNNER_READY=true/i.test(`${approvedAgentBuildRunnerReport}\n${approvedAgentBuildRunnerStep?.stdout ?? ""}`);
  const approvedAgentBuildRunnerDryRunReady = /Mode:\s*REPORT_ONLY/i.test(`${approvedAgentBuildRunnerDryRunReport}\n${approvedAgentBuildRunnerDryRunStep?.stdout ?? ""}`) && /Dry-run invocation never builds agents itself:\s*true/i.test(`${approvedAgentBuildRunnerDryRunReport}\n${approvedAgentBuildRunnerDryRunStep?.stdout ?? ""}`) && /Policy allows scoped docs\/register\/validator agent builds:\s*true/i.test(`${approvedAgentBuildRunnerDryRunReport}\n${approvedAgentBuildRunnerDryRunStep?.stdout ?? ""}`) && /Dry-run invocation never creates PRs itself:\s*true/i.test(`${approvedAgentBuildRunnerDryRunReport}\n${approvedAgentBuildRunnerDryRunStep?.stdout ?? ""}`) && /Never merges PRs:\s*true/i.test(`${approvedAgentBuildRunnerDryRunReport}\n${approvedAgentBuildRunnerDryRunStep?.stdout ?? ""}`) && /Never repairs files:\s*true/i.test(`${approvedAgentBuildRunnerDryRunReport}\n${approvedAgentBuildRunnerDryRunStep?.stdout ?? ""}`) && /Never deploys:\s*true/i.test(`${approvedAgentBuildRunnerDryRunReport}\n${approvedAgentBuildRunnerDryRunStep?.stdout ?? ""}`) && /APPROVED_AGENT_BUILD_RUNNER_DRY_RUN_READY=true/i.test(`${approvedAgentBuildRunnerDryRunReport}\n${approvedAgentBuildRunnerDryRunStep?.stdout ?? ""}`);
  const approvedAgentBuildRunnerMergeGateText = `${approvedAgentBuildRunnerMergeGateReport}\n${approvedAgentBuildRunnerMergeGateStep?.stdout ?? ""}`;
  const approvedAgentBuildRunnerMergeGateReady = /Mode:\s*REPORT_ONLY_MERGE_GATE/i.test(approvedAgentBuildRunnerMergeGateText) && /REPORT_ONLY_MERGE_GATE=true/i.test(approvedAgentBuildRunnerMergeGateText) && /GATE_CONFIGURATION_READY=true/i.test(approvedAgentBuildRunnerMergeGateText) && /MERGE_READY=false/i.test(approvedAgentBuildRunnerMergeGateText) && /MISSING_CHECKS_BLOCK_MERGE=true/i.test(approvedAgentBuildRunnerMergeGateText) && /SAFE_REPAIR_LIMITED=true/i.test(approvedAgentBuildRunnerMergeGateText) && /NEVER_MERGES=true/i.test(approvedAgentBuildRunnerMergeGateText) && /NEVER_DEPLOYS=true/i.test(approvedAgentBuildRunnerMergeGateText) && /APPROVED_AGENT_BUILD_RUNNER_MERGE_GATE_READY=true/i.test(approvedAgentBuildRunnerMergeGateText);
  ...
  assertCondition(checks, "Auto-repair decision check reported safe repair boundary", autoRepairDecisionReportOnly, autoRepairDecisionReportOnly ? "SAFE_DOCS_REGISTER_JSON_FORMAT_REPAIR_ALLOWED" : "not found or unsafe output");
