...
function main() {
  ...
  const approvedAgentBuildRunnerMergeGateReport = readTextSafe("scripts/wellfit-dev-agent/output/approved-agent-build-runner-merge-gate-report.md");
  ...
  const approvedAgentBuildRunnerMergeGateReady = /REPORT_ONLY_MERGE_GATE=true/i.test(`${approvedAgentBuildRunnerMergeGateReport}\n${approvedAgentBuildRunnerMergeGateStep?.stdout ?? ""}`) && /GATE_CONFIGURATION_READY=true/i.test(`${approvedAgentBuildRunnerMergeGateReport}\n${approvedAgentBuildRunnerMergeGateStep?.stdout ?? ""}`) && /APPROVED_AGENT_BUILD_RUNNER_MERGE_GATE_READY=true/i.test(`${approvedAgentBuildRunnerMergeGateReport}\n${approvedAgentBuildRunnerMergeGateStep?.stdout ?? ""}`) && /MAX_AGENTS_PER_RUN=1/i.test(`${approvedAgentBuildRunnerMergeGateReport}\n${approvedAgentBuildRunnerMergeGateStep?.stdout ?? ""}`) && /MISSING_CHECKS_BLOCK_MERGE=true/i.test(`${approvedAgentBuildRunnerMergeGateReport}\n${approvedAgentBuildRunnerMergeGateStep?.stdout ?? ""}`) && /SAFE_REPAIR_LIMITED=true/i.test(`${approvedAgentBuildRunnerMergeGateReport}\n${approvedAgentBuildRunnerMergeGateStep?.stdout ?? ""}`) && /MERGE_READY=false/i.test(`${approvedAgentBuildRunnerMergeGateReport}\n${approvedAgentBuildRunnerMergeGateStep?.stdout ?? ""}`) && /NEVER_MERGES=true/i.test(`${approvedAgentBuildRunnerMergeGateReport}\n${approvedAgentBuildRunnerMergeGateStep?.stdout ?? ""}`) && /NEVER_DEPLOYS=true/i.test(`${approvedAgentBuildRunnerMergeGateReport}\n${approvedAgentBuildRunnerMergeGateStep?.stdout ?? ""}`) && /Never creates agents automatically:\s*true/i.test(`${approvedAgentBuildRunnerMergeGateReport}\n${approvedAgentBuildRunnerMergeGateStep?.stdout ?? ""}`) && /Never opens PRs automatically:\s*true/i.test(`${approvedAgentBuildRunnerMergeGateReport}\n${approvedAgentBuildRunnerMergeGateStep?.stdout ?? ""}`) && /Never approves PRs:\s*true/i.test(`${approvedAgentBuildRunnerMergeGateReport}\n${approvedAgentBuildRunnerMergeGateStep?.stdout ?? ""}`);
  ...
  assertCondition(checks, "Approved agent build runner merge gate reported safely", approvedAgentBuildRunnerMergeGateReady, approvedAgentBuildRunnerMergeGateReady ? "REPORT_ONLY_MERGE_GATE=true, gate config ready, safe repair limited, missing checks block merge, MERGE_READY=false allowed" : "not found or unsafe output");
