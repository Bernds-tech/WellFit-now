# Reward Fairness Guard Report Agent

## Status

- Agent id: `reward-fairness-guard-report-agent`
- Planning priority for this batch: 3
- Risk level: `critical`
...
Critical guard prioritized report-only after the learning and Buddy tone foundations. The first planning version is a critical report-only guard: it creates documentation, register metadata, validator coverage, catalog visibility, and work-log evidence only.

## Scope

Reports fairness, abuse, pressure, and authority risks around reward concepts while never changing reward logic, mission completion, anti-cheat, ledger, points, XP, token, payment, or runtime behavior.

Allowed artifact scope in this step is limited to:

- `docs/architecture/WELLFIT_REWARD_FAIRNESS_GUARD_REPORT_AGENT.md`
- `project-register/reward-fairness-guard-report-agent.json`
- `scripts/wellfit-dev-agent/src/reward-fairness-guard-report-agent-check.mjs`
...
The validator at `scripts/wellfit-dev-agent/src/reward-fairness-guard-report-agent-check.mjs` checks that the register, architecture document, catalog entry, backlog entry, and work-log evidence agree on report-only status, runtime denial, protected boundaries, and required artifact paths.

## Next safe maintenance task

Keep `reward-fairness-guard-report-agent` synchronized across docs, register, validator, catalog, backlog, and build/work log. Do not implement runtime behavior from this artifact without a later explicit human-approved task.

## KI-Fortsetzungs-Prompt

Wenn du hier weiterarbeitest, lies zuerst `AGENTS.md`, `todolist/WORK_MAP.md`, `todolist/TODO_INDEX.md`, project-register/reward-fairness-guard-report-agent.json, project-register/approved-agent-build-backlog.json, project-register/agent-catalog.json, project-register/agent-work-log.json und scripts/wellfit-dev-agent/src/reward-fairness-guard-report-agent-check.mjs. Arbeite nur an report-only Dokumentation, Register-Metadaten, Katalog-/Backlog-/Work-Log-Synchronisierung und Validatoren fuer diesen Agenten. Keine App Runtime, keine Nutzerprofilierung, keine Firestore-Regeln, keine Firebase Functions, keine UI-Ausfuehrung, keine geschuetzten Produktentscheidungen, kein Merge, kein Deploy und keine Auto-Reparatur aktivieren.
