...
- Completed so far: recovered the successful textured/rigged Meshy production artifact and eight generated action clips; added walk/run extraction; reduced ten animation payloads from about 75 MB to under 1 MB total; integrated autonomous Rudi behavior with dialogue, directional movement, foreground/background depth, coffee/table/lounge props, a separately simulated cape and a static WebGL fallback. Owner video review identified that the former small fixed Canvas visually imprisoned Rudi and clipped his legs; the renderer is now viewport-wide, the model scale is reduced by about one third, safe viewport margins prevent edge clipping, and Rudi now walks between actions with a grounding shadow instead of sliding between poses.
- Evidence: targeted ESLint, `rudi:validate` and the complete Next.js production build succeed locally after the viewport-stage and locomotion corrections. Meshy generation and animation Actions runs succeeded on 2026-09-04. PR #401 prior exact head `cfed7908d44a7d838044df95b55167b61b6b9179` passed Build #1239, Container #224, Database #216, Beta 1 Emulator #195 and all Project Memory gates; fresh exact-head CI is pending. Supervised visual preview is incompatible with this retained Next.js dev command and did not run.
