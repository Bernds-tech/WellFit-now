...
- Completed so far: applied `--prefer-offline --no-audit --no-fund` consistently to root and Functions `npm ci` steps.
- Exact next step: run workflow/diff checks, push the branch and require exact-head CI.
