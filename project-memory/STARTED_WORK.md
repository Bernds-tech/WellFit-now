...
- Completed so far: recovered the successful textured/rigged Meshy production artifact and eight generated action clips; added walk/run extraction; reduced ten animation payloads from about 75 MB to under 1 MB total; integrated autonomous Rudi behavior with dialogue, directional movement, foreground/background depth, coffee/table/lounge props, a separately simulated cape and a static WebGL fallback. Successive owner reviews removed the small fixed box and exposed a scale override that kept resetting the nominally reduced avatar to `0.94`; the live animated scale is now actually `0.66`. Rudi renders through a body-level transparent overlay outside the scrolling main element, remains completely inside body-safe boundaries, stays with his content-world position until an edge is reached and then climbs, runs or jumps after the user. Travel intervals are shorter, CTA emotion uses real head/spine articulation, head/spine motion continues autonomously away from CTAs, the coffee follows the animated right hand and table/lounge props visibly rise and turn into their scenes.
- Evidence: targeted ESLint, `rudi:validate` and the complete Next.js production build succeed locally after the viewport-stage and locomotion corrections. PR #401 functional head `944565028fb533023aeb53952c1855f223f75b0f` passed Build #1244, Container #229, Database #221, Beta 1 Emulator #200 and all Project Memory gates. Meshy generation and animation Actions runs succeeded on 2026-09-04. Supervised visual preview is incompatible with this retained Next.js dev command and did not run.
- Still open: runnable browser/device visual acceptance of the corrected scale/body overlay/scroll-lag/CTA/table behavior, individually articulated finger bones, custom Text-to-Motion living-action artifact generation and public ChatGPT Site source synchronization.
...
- Status: VERIFIED
- Risk: R2
- Scope: admin-only bounded partner catalog projection with revision/lifecycle/inventory facts and privacy-minimal responses.
- Branch/PR: `codex/partner-catalog-admin-read-20260903` / PR pending.
- Work lock: `LOCK-WFN-PARTNER-CATALOG-ADMIN-READ-BASELINE`.
- Completed: admin-only catalog projection caps pages at 100, provides deterministic cursors and returns lifecycle/revision/inventory facts without audit actors or raw revision records.
...
- Exact next step: merge PR #398; keep detailed revision history as a separate bounded audit task.
- Owner action needed: none.
