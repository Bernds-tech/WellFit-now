...
- World-presence correction: owner live review then showed that viewport-following still felt like a separate window, the table was not apparent and the 3D version had lost login/register emotion. Rudi now holds his content-relative position during scroll, follows later through climb/run catch-up states, adds distance-based head/spine CTA attention and excitement without losing autonomous behavior, and presents a larger elevated table as the second scene.
...
- Result status: IMPLEMENTED_NOT_VERIFIED pending browser/device visual acceptance of the scroll-lag/CTA/table correction and separate public-Site synchronization.
