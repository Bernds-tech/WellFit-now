...
function callableErrorMessage(error: unknown): string {
  ...
  if (diagnostic.includes("bereits abgeschlossen") || diagnostic.includes("bereits belohnt")) return "Diese Wochenmission wurde in deiner aktuellen lokalen Kalenderwoche bereits abgeschlossen.";
  ...
  if (diagnostic.includes("nutzerlokalen woche") || diagnostic.includes("user-local week")) return "Diese Wochenmission gehört nicht mehr zu deiner aktuellen lokalen Kalenderwoche.";
...
function parseActiveAttempts(value: unknown, fallbackTimeZone: string): Beta1WeeklyActiveAttempt[] {
  ...
  return value.flatMap((entry) => {
    ...
    return [{
      ...
      timeZone: typeof item.timeZone === "string" ? item.timeZone : fallbackTimeZone,
...
function parseProgress(data: RawProgressResponse): Beta1WeeklyMissionProgress {
  ...
  return {
    ...
    activeAttempts: parseActiveAttempts(data.activeAttempts, data.timeZone),
...
export async function fetchWeeklyMissionProgress(): Promise<Beta1WeeklyMissionProgress> {
  requireSignedInUser();
  try {
    const callable = httpsCallable<{ timeZone: string }, RawProgressResponse>(
      getFunctions(),
...
export async function submitWeeklyMissionForReview(missionId: string): Promise<Beta1WeeklyMissionActionResult> {
  ...
  try {
    ...
    const result = await callable({
      ...
      clientVersion: "weekly-missions-beta1-runtime-v2",
      appSessionId,
    ...
    return {
      ...
      message: data.reviewStatus === "approved"
        ...
        : "Wochenmission wurde für deine lokale Kalenderwoche serverseitig gestartet. Deine Bestätigung wartet auf Admin-Prüfung; es wurden noch keine WFXP gutgeschrieben.",
      rewardXp: 0,
...
async function completeWeeklyMission(attemptId: string): Promise<{
  ...
  const callable = httpsCallable<{ attemptId: string; timeZone: string }, CompleteWeeklyMissionResponse>(
    getFunctions(),
