...
export function generateLocalAiMission(profile: AiMissionProfile = {}): GeneratedMission {
  ...
  if (goal === "lernen") {
    ...
    return {
      title: `KI Wissens-Sprint ${slotIndex + 1}`,
      category: "Tagesmissionen",
      description: `Löse ${questionTarget} kurze Wissensfragen. Bei stabilen Erfolgen wird die Schwierigkeit Schritt für Schritt erhöht.`,
      difficulty: questionTarget >= 6 || level > 3 ? "Mittel" : "Leicht",
      type: "Community",
      duration: "10 Minuten",
      reward: clampReward(15 + level * 2 + slotIndex * 2),
  ...
  if (goal === "abnehmen") {
    ...
    return {
      title: lowActivity ? `KI Sanfter Fettstoffwechsel-Start ${slotIndex + 1}` : `KI Aktiv-Burn Mission ${slotIndex + 1}`,
      category: "Tagesmissionen",
      description: adaptive.nearLimit
        ? `Du bist nahe an deinem aktuellen Belastungslimit. Wir halten das Ziel bei rund ${baseTarget.toLocaleString("de-DE")} Schritten und erhöhen danach wieder langsam.`
        : `Dein Ziel steigt kontrolliert: erst kleine Schritte, dann etwas stärker. Heute: ${baseTarget.toLocaleString("de-DE")} echte Schritte.`,
      difficulty: adaptive.nearLimit ? "Mittel" : lowActivity ? "Leicht" : "Mittel",
      type: "Bewegung",
      duration: "1 Tag",
      reward: clampReward(20 + slotIndex * 4 + Math.round(baseTarget / 1000) * 2),
  ...
  return {
    title: lowActivity ? `KI Tagesstart Bewegung ${slotIndex + 1}` : `KI Fortschrittsrunde ${slotIndex + 1}`,
    category: "Tagesmissionen",
    description: adaptive.nearLimit
      ? `Du näherst dich deiner persönlichen Grenze. Das System stabilisiert dein Ziel bei etwa ${movementTarget.toLocaleString("de-DE")} Schritten und steigert danach langsamer weiter.`
      : `Das Ziel wächst in kleinen Schritten mit dir: zuerst ${adaptive.lastTarget}, dann ${adaptive.nextTarget}. Heute entspricht das etwa ${movementTarget.toLocaleString("de-DE")} Schritten.`,
    difficulty: adaptive.nearLimit ? "Mittel" : lowActivity ? "Leicht" : "Mittel",
    type: "Bewegung",
    duration: "1 Tag",
    reward: clampReward(18 + slotIndex * 3 + Math.round(movementTarget / 1000) * 2),
    targetValue: movementTarget,
    unit: "steps",
  };
}
...
export async function createAiMissionForCurrentUser(profile: AiMissionProfile = {}) {
  ...
  await setDoc(doc(db, "aiGeneratedMissions", missionId), {
    ...mission,
