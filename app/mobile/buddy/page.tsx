      ...
      <section className="px-4 pt-4">
        <div className="rounded-[30px] bg-[#04343b] p-5 shadow-[0_12px_30px_rgba(0,0,0,0.18)]">
          <div className="flex items-center justify-between gap-3">
            ...
            <span className={`rounded-full px-2.5 py-1 text-[10px] font-black ${serverReady ? "bg-emerald-400/15 text-emerald-200" : "bg-amber-300/15 text-amber-100"}`}>
              {serverReady ? "Server-Authority" : "Server ausstehend"}
            </span>
          </div>
          <div className="mt-3 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-yellow-200">
                {isSavingBuddy ? "Server-Aktion läuft..." : `Level ${buddy.level} · ${buddy.points} WFXP`}
              </p>
            </div>
          </div>
          ...
          {visibleActions.map((action) => (
            ...
            >
              <div className="flex items-center justify-between gap-3">
                ...
                <span className="text-xs font-black">{action.cost > 0 ? `${action.cost} WFXP` : "frei"}</span>
              </div>
