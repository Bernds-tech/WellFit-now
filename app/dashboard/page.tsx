...
const readCachedUser = (uid: string): User | null => {
  try { const cached = localStorage.getItem(getDashboardCacheKey(uid)); return cached ? normalizeUser(JSON.parse(cached) as Partial<User>, uid) : null; }
  catch (error) { console.error("Dashboard Cache konnte nicht gelesen werden", error); return null; }
};
...
export default function DashboardPage() {
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, async (firebaseUser) => {
      ...
      unsubscribeSnapshot = onSnapshot(userRef, async (userSnap) => {
        if (userSnap.exists()) {
          ...
          setUser(normalizedUser); writeCachedUser(normalizedUser); setLoadedFromCache(false); setIsRealtimeConnected(true); setMessage("Dashboard live synchronisiert."); setIsLoadingUser(false); return;
        }
  ...
  useEffect(() => {
    const handleFocus = () => { const savedPermissions = localStorage.getItem("wellfit-permissions"); if (savedPermissions) { try { const parsed = JSON.parse(savedPermissions); setPermissions({ location: !!parsed.location }); } catch (error) { console.error("Fehler beim Aktualisieren der Permissions", error); } } };
    window.addEventListener("focus", handleFocus); return () => window.removeEventListener("focus", handleFocus);
  }, []);
      ...
      <div className="flex h-full">
        ...
        <section className="relative flex h-full flex-1 flex-col overflow-hidden px-10 py-7 pb-0">
          ...
          <div className="grid flex-1 grid-cols-3 gap-6 overflow-y-auto pb-28 pr-2">
            ...
            <div className="rounded-[30px] bg-[#053841]/85 p-6"><h2 className="mb-4 text-3xl font-bold text-cyan-300">Heutige Missionen</h2><p className="text-xl text-white/80">Keine Favoriten markiert. Gehe zu den Missionen!</p></div>
            <div className="rounded-[30px] bg-[#053841]/85 p-6"><h2 className="mb-4 text-3xl font-bold text-cyan-300">Punkte</h2><p className="text-6xl font-extrabold leading-tight">{(isLoadingUser && !user) ? "..." : `${pointsBalance} Punkte`}</p><p className="mt-4 text-2xl text-white/70">Dein Fortschritt</p></div>
            <div className="rounded-[30px] bg-[#053841]/85 p-6"><h2 className="mb-4 text-3xl font-bold text-cyan-300">Mein Begleiter</h2><div className="text-6xl">🐉</div><p className="mt-4 text-2xl">Energie: {(isLoadingUser && !user) ? "..." : `${buddyEnergy}%`}</p><p className="mt-2 text-2xl">Hunger: {(isLoadingUser && !user) ? "..." : `${buddyHunger}%`}</p><p className="mt-2 text-xl text-white/70">Futter kostet: {foodPrice} Punkte</p><button type="button" onClick={feedBuddy} disabled={isLoadingUser && !user} className={`mt-4 rounded-xl px-4 py-3 font-bold ${(isLoadingUser && !user) ? "cursor-not-allowed bg-gray-500" : "bg-cyan-600 hover:bg-cyan-700"}`}>Flammi füttern</button></div>
            <div className="rounded-[30px] bg-[#053841]/85 p-6"><h2 className="mb-4 text-3xl font-bold text-cyan-300">Economy</h2><p className="text-2xl text-white/90">Reserve: {economyConfig.reserve}</p><p className="text-2xl text-white/90">Umlauf: {economyConfig.circulating}</p></div>
