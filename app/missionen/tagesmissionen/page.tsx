...
type MissionProgress = { missionId: string; status: string; progressValue: number; pointsGranted?: number; lockedRewardPoints?: number; avatarMultiplier?: number; userEconomyMultiplier?: number; systemRewardRate?: number; precisionFactor?: number; socialMultiplier?: number; streakMultiplier?: number; sponsorMultiplier?: number; integrityMultiplier?: number; };
  ...
  useEffect(() => {
    return onAuthStateChanged(auth, (firebaseUser) => {
      ...
      const unsubscribeProgress = onSnapshot(progressQuery, (snapshot) => {
        ...
        snapshot.docs.forEach((docSnap) => { const data = docSnap.data(); const missionId = String(data.missionId ?? ""); if (!missionId) return; progressMap[missionId] = { missionId, status: String(data.status ?? "open"), progressValue: Number(data.progressValue ?? 0), pointsGranted: Number(data.pointsGranted ?? 0), lockedRewardPoints: Number(data.lockedRewardPoints ?? 0), avatarMultiplier: Number(data.avatarMultiplier ?? 1), userEconomyMultiplier: Number(data.userEconomyMultiplier ?? 1), systemRewardRate: Number(data.systemRewardRate ?? data.rewardRateAtStart ?? 1), precisionFactor: Number(data.precisionFactor ?? 1), socialMultiplier: Number(data.socialMultiplier ?? 1), streakMultiplier: Number(data.streakMultiplier ?? 1), sponsorMultiplier: Number(data.sponsorMultiplier ?? 1), integrityMultiplier: Number(data.integrityMultiplier ?? 1) }; });
