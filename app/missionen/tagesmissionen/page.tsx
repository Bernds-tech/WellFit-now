...
export default function MissionenPage() {
  ...
  const MissionTile = ({ mission }: { mission: Mission }) => {
    ...
    return (
      <button draggable onDragStart={(event) => event.dataTransfer.setData("text/plain", mission.id)} onClick={() => setSelectedMissionId(mission.id)} className={`group relative h-[118px] w-[130px] shrink-0 overflow-hidden rounded-[10px] border bg-[#044b54]/95 p-2 text-left shadow-[0_4px_14px_rgba(0,0,0,0.25)] transition duration-300 hover:-translate-y-1 hover:bg-[#075d68] active:scale-95 ${isRecommended ? "border-yellow-400 ring-2 ring-yellow-300/70" : selectedMissionId === mission.id ? "border-yellow-400" : "border-cyan-300/25"}`}>
        ...
        <span onClick={(event) => { event.preventDefault(); event.stopPropagation(); toggleFavorite(mission.id); }} className={`absolute right-2 top-1 z-20 text-2xl transition hover:scale-150 ${isFavorite ? "text-yellow-400" : "text-white/25 hover:text-yellow-300"}`}>★</span>
        ...
        <div className="absolute bottom-2 left-2 right-2 flex items-center gap-1 text-sm font-bold text-white"><Image src="/coin.png" alt="Punkte" width={25} height={25} className="rounded-full" /><span>+ {mission.reward} Pkt.</span></div>
      </button>
    );
  };
    ...
    <main className="h-screen w-screen overflow-hidden text-white" style={{ background: `linear-gradient(to bottom right, rgba(0,170,190,${brightness / 100}), rgba(0,80,90,1))` }}>
      <div className="flex h-full">
        ...
        <section className="relative flex h-full flex-1 flex-col overflow-hidden px-7 py-5 pb-0">
          <div className="mb-4 flex justify-between gap-4"><div><h1 className="text-5xl font-extrabold leading-none">Tagesmissionen</h1><p className="mt-1 text-lg text-cyan-100/90">Ziehe Missionen in deine 3 Tagesfelder.</p><p className="mt-1 text-xs text-cyan-100/65">10 balancierte Tagesmissionen · Punkte-System aktiv</p></div><div className="flex items-center gap-3"><div className="rounded-2xl border border-yellow-400/40 bg-[#073b44] px-4 py-2"><p className="text-sm font-bold text-yellow-300">🔥 Streak 0</p><p className="text-[10px] text-white/55">Best 0</p></div><div className="rounded-2xl border border-cyan-300/30 bg-[#073b44] px-4 py-2"><p className="text-sm font-bold text-cyan-100">🎯 Heute 0/3</p><p className="text-[10px] text-white/55">Noch 3 bis Bonus</p></div><div className="rounded-2xl border border-green-300/30 bg-[#073b44] px-4 py-2"><p className="text-sm font-bold text-green-300">🌈 Vielfalt {diversityCount}/3</p><p className="text-[10px] text-white/55">3 Typen = Bonus</p></div></div></div>
          ...
          <div className="grid min-h-0 flex-1 grid-cols-[2.15fr_1fr] gap-5 overflow-hidden pb-20">
            <div className="flex min-h-0 flex-col overflow-hidden">
              ...
              <div className="mt-4 min-h-0 flex-1 overflow-y-auto pr-1"><section className="rounded-[8px] bg-[#00606b]/75 px-4 py-3"><h2 className="mb-2 w-fit border-b border-white/70 text-2xl font-extrabold tracking-wide">Favoriten</h2><div className="flex gap-3 overflow-x-auto pb-2">{favoriteMissions.length ? favoriteMissions.map((mission) => <MissionTile key={`fav-${mission.id}`} mission={mission} />) : <div className="rounded-xl border border-dashed border-white/25 px-5 py-5 text-sm text-white/50">Noch keine Favoriten. Tippe auf einen Stern.</div>}</div></section><section className="mt-4 rounded-[8px] bg-[#00606b]/75 px-4 py-3"><div className="mb-3 flex items-center justify-between"><h2 className="w-fit border-b border-white/70 text-3xl font-extrabold tracking-wide">Missionspool</h2><span className="rounded-full border border-yellow-400/50 bg-yellow-400/10 px-3 py-1 text-xs font-bold text-yellow-200">KI empfiehlt gelb</span></div><div className="grid grid-cols-[repeat(auto-fill,minmax(130px,1fr))] gap-3">{missions.map((mission) => <MissionTile key={mission.id} mission={mission} />)}</div></section></div>
            </div>
            <aside className="h-full overflow-hidden rounded-[6px] bg-[#003d46]/95 p-5 shadow-[0_12px_30px_rgba(0,0,0,0.2)]">
              <div className="flex h-full flex-col overflow-y-auto pr-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"><div className="mb-4 flex items-start justify-between"><h2 className="text-2xl font-extrabold tracking-wide">Missionsdetails</h2><button onClick={() => toggleFavorite(selectedMission.id)} className={`text-5xl leading-none transition hover:scale-125 ${favoriteIds.includes(selectedMission.id) ? "text-yellow-400" : "text-white/25 hover:text-yellow-300"}`}>★</button></div><div className="flex justify-center text-6xl text-cyan-300">{missionIcon(selectedMission.type)}</div><h3 className="mt-3 text-center text-3xl font-extrabold leading-tight">{selectedMission.title}</h3><p className="mt-3 text-center text-base leading-tight text-white/90">{selectedMission.description}</p><div className="mt-4"><div className="mb-1 flex justify-between text-sm text-white/75"><span>Fortschritt</span><span className="font-bold">0%</span></div><div className="h-5 overflow-hidden rounded bg-[#062e34]"><div className="h-full rounded bg-cyan-300 transition-all duration-700" style={{ width: "0%" }} /></div></div><div className="mt-3 flex items-center justify-between text-base"><span>Schwierigkeit</span><span className="font-bold">{selectedMission.difficulty}</span></div><div className="mt-2 flex items-center gap-3 text-lg font-bold"><Image src="/coin.png" alt="Punkte" width={34} height={34} className="rounded-full" /><span>+ {currentReward} Punkte</span></div><div className="mt-3 overflow-hidden rounded-xl border border-yellow-500/30 bg-yellow-500/10 text-xs text-white/80"><button onClick={() => setRewardDetailsOpen((open) => !open)} className="flex w-full items-center justify-between p-3 text-left font-bold text-yellow-300"><span>So entstehen die Punkte</span><span>{rewardDetailsOpen ? "−" : "+"}</span></button>{rewardDetailsOpen && <div className="border-t border-yellow-500/20 px-3 pb-3"><div className="mt-2 grid grid-cols-2 gap-1"><span>Basis</span><span className="text-right">{selectedMission.reward}</span><span>Vielfalt</span><span className="text-right">×{diversityMultiplier.toFixed(2)}</span><span>Anti-Farming</span><span className="text-right">×{antiFarmingMultiplier.toFixed(2)}</span></div><p className="mt-2 text-cyan-50/80">Abwechslung erhöht die Punkte. Wiederholung senkt sie.</p></div>}</div><button className="mt-4 w-full rounded-[16px] bg-blue-600 px-4 py-3 text-lg font-extrabold transition hover:bg-blue-700 active:scale-95">Mission starten</button></div>
            </aside>
          </div>
