...
type HistoryStatus = "geschafft" | "nicht geschafft" | "in PrÃ¼fung";
type HistoryEntry = { id: string; title: string; category: "Tagesmissionen" | "Wochenmissionen" | "Abenteuer" | "Challenge" | "WettkÃ¤mpfe" | string; rewardLabel: string; completedAt: string; icon: string; pointsDelta?: number; status?: HistoryStatus; };
...
export default function HistoryPage() {
  ...
  const clearHistory = () => { setHistoryMessage("LÃ¶schen ist spÃ¤ter serverseitig geplant. Firestore-History bleibt vorerst unverÃ¤ndert."); };
    ...
    <main className="h-screen w-screen overflow-hidden text-white" style={{ background: `linear-gradient(to bottom right, rgba(0,170,190,${brightness / 100}), rgba(0,80,90,1))` }}>
      <div className="flex h-full">
        ...
        <section className="relative flex h-full flex-1 flex-col overflow-hidden px-7 py-5 pb-0">
          ...
          <div className="mb-4 flex gap-3"><button onClick={() => setStatusFilter("Alle")} className={`rounded-xl px-4 py-2 text-sm font-bold ${statusFilter === "Alle" ? "bg-cyan-300 text-black" : "bg-[#053841]/90 text-white/80"}`}>Alle</button><button onClick={() => setStatusFilter("geschafft")} className={`rounded-xl px-4 py-2 text-sm font-bold ${statusFilter === "geschafft" ? "bg-green-300 text-black" : "bg-[#053841]/90 text-white/80"}`}>Geschafft</button><button onClick={() => setStatusFilter("nicht geschafft")} className={`rounded-xl px-4 py-2 text-sm font-bold ${statusFilter === "nicht geschafft" ? "bg-red-300 text-black" : "bg-[#053841]/90 text-white/80"}`}>Nicht geschafft</button><button onClick={() => setStatusFilter("in PrÃ¼fung")} className={`rounded-xl px-4 py-2 text-sm font-bold ${statusFilter === "in PrÃ¼fung" ? "bg-yellow-300 text-black" : "bg-[#053841]/90 text-white/80"}`}>In PrÃ¼fung</button></div>
          <div className="min-h-0 flex-1 overflow-y-auto pb-20 pr-2">
            {visibleHistory.length === 0 ? (<div className="rounded-[22px] bg-[#053841]/90 p-6 text-center shadow-[0_12px_30px_rgba(0,0,0,0.18)]"><div className="text-5xl">ðŸ“œ</div><h2 className="mt-3 text-3xl font-bold">Noch keine EintrÃ¤ge</h2><p className="mt-2 text-base text-white/75">Sobald du Missionen abschlieÃŸt oder nicht schaffst, erscheinen sie hier.</p><p className="mt-2 text-sm text-white/55">Dieser Verlauf wird live aus Firestore geladen und gerÃ¤teÃ¼bergreifend synchronisiert.</p></div>) : (<div className="space-y-3">{visibleHistory.map((entry) => { const status = entry.status ?? "geschafft"; return (<div key={entry.id} className="rounded-[20px] border border-cyan-300/10 bg-[#053841]/90 p-4 shadow-[0_8px_20px_rgba(0,0,0,0.14)]"><div className="flex items-center justify-between gap-4"><div className="flex items-center gap-4"><div className="flex h-12 w-12 items-center justify-center rounded-xl bg-[#0a3d46] text-3xl">{entry.icon}</div><div><h3 className="text-lg font-bold text-white">{entry.title}</h3><p className="mt-1 text-sm text-cyan-200">{entry.category}</p><p className="mt-1 text-xs text-white/60">{status === "geschafft" ? "Abgeschlossen" : status} am {new Date(entry.completedAt).toLocaleString("de-DE")}</p></div></div><div className="flex items-center gap-3"><div className={`rounded-xl border px-3 py-2 text-sm font-bold ${status === "geschafft" ? "border-green-400/40 bg-green-400/10 text-green-300" : status === "nicht geschafft" ? "border-red-400/40 bg-red-400/10 text-red-300" : "border-yellow-400/40 bg-yellow-400/10 text-yellow-300"}`}>{status}</div><div className="rounded-xl border border-yellow-500/30 bg-yellow-500/10 px-3 py-2 text-sm font-bold text-yellow-400">{entry.rewardLabel}</div></div></div></div>); })}</div>)}
          </div>
          <div className="absolute bottom-0 left-0 right-0 flex items-center justify-between border-t border-cyan-400/10 bg-[#062f35]/95 px-5 py-3"><div className="flex items-center gap-3"><div className="min-w-[150px] rounded-xl border border-cyan-400/10 bg-[#041f24] px-3 py-2"><p className="text-[10px] uppercase text-white/50">EintrÃ¤ge gesamt</p><p className="mt-1 text-sm font-semibold text-white">{sortedHistory.length}</p></div><div className="min-w-[150px] rounded-xl border border-yellow-500/60 bg-[#041f24] px-3 py-2 text-center"><p className="text-[10px] uppercase text-white/50">Letzter Eintrag</p><p className="mt-1 text-lg font-bold text-white">{sortedHistory[0]?.icon ?? "â€”"}</p></div><div className="min-w-[150px] rounded-xl border border-cyan-400/10 bg-[#041f24] px-3 py-2 text-center"><p className="text-[10px] uppercase text-white/50">Speicher</p><p className="mt-1 text-sm font-semibold text-white">{storageMode}</p></div></div><div className="flex items-center gap-3 text-xl text-white/80"><span>f</span><span>ð•</span><span>in</span></div></div>
        </section>
      </div>
    </main>
  );
