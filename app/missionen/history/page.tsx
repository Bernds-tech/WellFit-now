...
const sanitizeRewardLabel = (value: unknown, pointsDelta?: number) => {
  ...
  return value.replace(/WFT/gi, "interne Punkte").replace(/NFT/gi, "Beta-Abzeichen").replace(/Token/gi, "interne Punkte").trim();
};
...
export default function HistoryPage() {
  ...
  useEffect(() => {
    const unsubscribeAuth = onAuthStateChanged(auth, (firebaseUser) => {
      if (!firebaseUser) {
        setHistoryEntries([]);
        setHistoryMessage("Bitte einloggen, um deine Mission-History zu sehen.");
        setStorageMode("Nicht eingeloggt");
        return;
      }
      ...
      const unsubscribeHistory = onSnapshot(historyQuery, (snapshot) => {
        const entries = snapshot.docs.map((docSnap) => {
          ...
          return { id: docSnap.id, title: String(data.title ?? "Mission"), category: String(data.category ?? "Mission"), rewardLabel: sanitizeRewardLabel(data.rewardLabel, pointsDelta), completedAt: toDateString(data.createdAt), icon: String(data.icon ?? "✅"), pointsDelta, status: String(data.status ?? "geschafft") as HistoryStatus };
        });
        setHistoryEntries(entries);
        setHistoryMessage(entries.length > 0 ? "Live synchronisierte Missions-History" : "Noch keine erledigten Missionen in Firestore.");
        setStorageMode("Firestore Live");
      }, (error) => {
        console.error("Firestore History konnte nicht geladen werden", error);
        setHistoryMessage("History konnte nicht live geladen werden.");
        setStorageMode("Firestore Fehler");
      });
    ...
    <main className="h-screen w-screen overflow-hidden text-white" style={{ background: `linear-gradient(to bottom right, rgba(0,170,190,${brightness / 100}), rgba(0,80,90,1))` }}>
      <div className="flex h-full">
        ...
        <section className="relative flex h-full flex-1 flex-col overflow-hidden px-7 py-5 pb-0">
          <div className="mb-4 flex justify-between"><div><h1 className="text-5xl font-extrabold leading-none">History</h1><p className="mt-1 text-lg text-cyan-100/90">{historyMessage}</p><p className="mt-1 text-sm text-white/60">Verlauf für interne Beta-Punkte und Missionsstatus. Keine Token, keine NFTs, keine Auszahlung.</p></div><div className="flex items-center gap-2"><button onClick={clearHistory} className="rounded-full border border-red-400/30 bg-red-500/10 px-4 py-2 text-sm font-bold text-red-300 hover:bg-red-500/20">History löschen</button><div className="rounded-full bg-[#073b44] px-4 py-2 text-sm">Verlauf</div></div></div>
