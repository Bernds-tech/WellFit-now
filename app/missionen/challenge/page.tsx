...
type Challenge = { id: number; title: string; category: ChallengeCategory; description: string; playersActive: number; localTokens: string; level: string; reward: string; icon: string; lat: number; lng: number; };
...
const challenges: Challenge[] = [
  { id: 1, title: "Balance-Park Challenge", category: "Sport & Bewegung", description: "Balanciere Ã¼ber Hindernisse und halte deine Kontrolle bis zum Ziel.", playersActive: 8, localTokens: "38,50", level: "2+", reward: "1,2 km", icon: "ðŸƒ", lat: 48.204994, lng: 16.386607 },
  { id: 2, title: "Fitness-Duelle", category: "Fitness & Klarheit", description: "Miss dich im lokalen Fitness-Quiz und sammle Bonus-Token vor Ort.", playersActive: 6, localTokens: "41,00", level: "3+", reward: "2,1 km", icon: "ðŸ‹ï¸", lat: 48.198123, lng: 16.371512 },
  { id: 3, title: "Mathe-Speed", category: "Wissen & Klarheit", description: "LÃ¶se so viele Rechenaufgaben wie mÃ¶glich in 45 Sekunden.", playersActive: 14, localTokens: "72,80", level: "4+", reward: "2,6 km", icon: "âœ…", lat: 48.210033, lng: 16.363449 },
  { id: 4, title: "Reaktions-Test", category: "Geschicklichkeit", description: "Tippe prÃ¤zise, reagiere schnell und knackt die Zielzeit.", playersActive: 5, localTokens: "29,40", level: "2+", reward: "1,4 km", icon: "âš¡", lat: 48.215115, lng: 16.396277 },
  { id: 5, title: "AR-FundstÃ¼ck", category: "AR & Erlebnis", description: "Entdecke versteckte Marker in deiner Umgebung und sichere Spezialpunkte.", playersActive: 9, localTokens: "64,20", level: "5+", reward: "3,0 km", icon: "ðŸ“", lat: 48.205914, lng: 16.357956 },
  { id: 6, title: "Mindset-Flow", category: "Wellness & Mindset", description: "Atme bewusst, bleibe fokussiert und schlieÃŸe den Ruhe-Parcours ab.", playersActive: 4, localTokens: "21,70", level: "1+", reward: "0,8 km", icon: "ðŸ§˜", lat: 48.20849, lng: 16.37208 },
];
...
export default function ChallengePage() {
  ...
  useEffect(() => { const savedUser = localStorage.getItem("wellfit-user"); const savedFavorites = localStorage.getItem("wellfit-favorite-challenges"); if (savedUser) { try { setUser(JSON.parse(savedUser)); } catch (error) { console.error("Fehler beim Laden des Users", error); } } if (savedFavorites) { try { setFavoriteIds(JSON.parse(savedFavorites)); } catch (error) { console.error("Fehler beim Laden der Challenge-Favoriten", error); } } }, []);
  ...
  const startRoute = () => { setMessage(`Route gestartet: ${selectedChallenge.title}`); localStorage.setItem("wellfit-active-challenge", JSON.stringify(selectedChallenge.id)); };
        ...
        <section className="relative flex h-full flex-1 flex-col overflow-hidden px-7 py-5 pb-0">
          ...
          <div className="grid min-h-0 flex-1 grid-cols-[2fr_0.95fr] gap-4 overflow-hidden pb-20">
            <div className="min-h-0 overflow-hidden rounded-[22px] border border-cyan-300/10 bg-[#0a5564]/55">
              <div className="flex border-b border-white/10 bg-[#093f4a]/95">{categories.map((category) => (<button key={category} onClick={() => setSelectedCategory(category)} className={`flex-1 border-r border-white/10 px-2 py-3 text-center text-sm font-semibold transition last:border-r-0 ${selectedCategory === category ? "bg-[#0e6e7a] text-cyan-200" : "text-white/75 hover:bg-white/5 hover:text-white"}`}>{category}</button>))}</div>
              <GoogleMissionMap title="Google Maps Challenge" subtitle="Zoomen Â· ziehen Â· lokale Challenges wÃ¤hlen" markers={challengeMarkers} selectedMarkerId={selectedChallengeId} onSelectMarker={setSelectedChallengeId} zoom={13} minHeightClassName="h-[calc(100%-45px)] min-h-[480px]" />
            </div>
            <div className="min-h-0 overflow-y-auto rounded-[22px] border border-cyan-300/10 bg-[#053841]/95 p-5 shadow-[0_10px_28px_rgba(0,0,0,0.18)]"><div className="mb-4 flex items-start justify-between gap-3"><div className="rounded-lg bg-cyan-500/20 px-3 py-2 text-sm font-bold text-cyan-300">{selectedChallenge.category}</div><button onClick={() => toggleFavorite(selectedChallenge.id)} className={`text-2xl ${favoriteIds.includes(selectedChallenge.id) ? "text-yellow-400" : "text-white/30"}`}>â˜…</button></div><div className="mb-4 flex items-center gap-3"><div className="flex h-12 w-12 items-center justify-center rounded-full bg-cyan-400/20 text-3xl">{selectedChallenge.icon}</div><h2 className="text-2xl font-bold text-white">{selectedChallenge.title}</h2></div><p className="mb-4 text-sm leading-relaxed text-white/85">{selectedChallenge.description}</p><div className="mb-5 h-2 w-full overflow-hidden rounded-full bg-black/25"><div className="h-full rounded-full bg-cyan-300" style={{ width: "32%" }} /></div><div className="space-y-2 text-sm text-white/80"><p>ðŸ‘¥ Vor Ort aktiv: {selectedChallenge.playersActive}</p><p>ðŸª™ Token vor Ort: {selectedChallenge.localTokens}</p><p>ðŸ“¶ Level-Empfehlung: {selectedChallenge.level}</p><p>ðŸŽ Belohnungen: {selectedChallenge.reward}</p></div><button onClick={startRoute} className="mt-5 w-full rounded-xl bg-cyan-500 px-4 py-3 text-base font-bold text-black transition hover:bg-cyan-400">Route</button></div>
          </div>
