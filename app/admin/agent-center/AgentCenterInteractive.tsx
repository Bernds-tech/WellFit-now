...
export default function AgentCenterInteractive({ data, firstRunOutput }: { data: Data; firstRunOutput?: unknown }) {
  ...
  async function createTaskProposalFromInbox(row: Row) {
    const inboxId = String(row.inboxId || "");
    if (!inboxId) return setFeedback("Kein inboxId vorhanden.");
    const result = await beta1AdminClient.createAgentTaskProposalFromApprovedInboxItem({ inboxId });
    if (result.accepted) {
      setFeedback(`Task Proposal erstellt: ${String((result as { taskProposalId?: string }).taskProposalId || "ok")}`);
      await refreshInbox();
      return;
    }
    setFeedback(result.message || "Task Proposal konnte nicht erstellt werden.");
  }
