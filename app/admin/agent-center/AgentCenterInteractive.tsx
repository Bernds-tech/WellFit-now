...
type Row = Record<string, unknown> & { id?: string; title?: string; name?: string; sourceLabel?: string; status?: string; canDecide?: boolean; decisionHint?: string; mirrorTargetId?: string };
type Data = { agents: Row[]; missions: Row[]; stats: Record<string, number> };

const FILTER_TO_BUCKET: Record<AdminCenterListFilter, "total" | ReturnType<typeof getAgentStatusBucket>> = {
  agent_total: "total",
  agent_pending: "pending_approval",
  agent_approved: "approved_ready",
  agent_rejected: "rejected",
  agent_blocked: "blocked",
  agent_in_progress: "in_progress",
  agent_completed: "completed",
  agent_repair_required: "repair_required",
  agent_halted_waiting_owner: "halted_waiting_owner",
  agent_cycle_restart_required: "cycle_restart_required",
  mission_total: "total",
  mission_pending: "pending_approval",
  mission_approved: "approved_ready",
...
export default function AgentCenterInteractive({ data }: { data: Data }) {
  ...
  async function decide(kind:"agent"|"mission", action: "approve"|"reject"|"revise"|"block"|"review", row: Row) {
    if (!row.canDecide) return setFeedback(String(row.decisionHint || "Noch nicht in Firestore-Mirror synchronisiert; erst Inbox-Sync nötig."));
    const targetId = String(row.mirrorTargetId || row.id || "");
    if(!targetId) return setFeedback("targetId fehlt.");
  ...
  return <section className="space-y-4 rounded-xl border border-white/12 bg-slate-950/35 p-4"><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-7">{FILTERS.map(i=><button key={i.key} onClick={()=>setActive(i.key)} className={`rounded-lg border p-3 text-left ${active===i.key?"border-cyan-300/60 bg-cyan-400/10":"border-white/12 bg-slate-900/55"}`}><p className="text-[0.68rem] uppercase tracking-[0.16em] text-cyan-100/65">{i.label}</p><p className="font-mono text-2xl font-bold text-white">{data.stats[i.statKey] ?? 0}</p></button>)}</div>{feedback?<p className="rounded border border-cyan-200/30 bg-cyan-400/10 px-3 py-2 text-xs text-cyan-50">{feedback}</p>:null}{activeStat > 0 && visible.length === 0 ? <p className="rounded border border-amber-200/30 bg-amber-400/10 px-3 py-2 text-xs text-amber-50">Statistik und Liste sind nicht synchron. Bitte Datenquellen prüfen.</p> : null}<div className="space-y-2 text-xs text-white/85">{visible.length === 0 ? <p className="rounded border border-white/10 bg-slate-900/40 px-3 py-2 text-white/75">Keine Einträge für diesen Filter vorhanden.</p> : visible.map((row)=>{const t=deriveTimeline(row); const name=String(row.title||row.name||row.id||"Eintrag"); const mission=active.startsWith("mission_"); return <div key={String(row.id||name)} className="rounded border border-white/10 p-2"><b>{name}</b> · {String(row.status||"—")}<div className="mt-1 grid grid-cols-2 gap-1"><span>Quelle: {String(row.sourceLabel || row.source || "—")}</span><span>dossierId: {String(row.sourceDossierId||row.dossierId||"—")}</span><span>adminDecision: {String(row.adminDecision||"—")}</span><span>taskProposalId: {String(row.taskProposalId||row.proposalId||row.id||"—")}</span><span>taskStatus: {String(row.taskStatus||row.status||"—")}</span><span>nextAction: {String(row.nextAction||"—")}</span><span>runnerEligibility: {String(row.runnerEligibility||"—")}</span><span>riskLevel: {String(row.riskLevel||"—")}</span><span>Erstellt: {formatAdminDate(t.createdAt)}</span><span>Submitted: {formatAdminDate(t.submittedAt)}</span><span>Waiting approval: {formatAdminDate(t.waitingForApprovalAt)}</span><span>Approved: {formatAdminDate(t.approvedAt)}</span><span>Rejected: {formatAdminDate(t.rejectedAt)}</span><span>Blocked: {formatAdminDate(t.blockedAt)}</span></div><div className="mt-2 flex flex-wrap gap-2"><button disabled={!row.canDecide} title={row.canDecide ? "" : String(row.decisionHint || "Noch nicht in Firestore-Mirror synchronisiert; erst Inbox-Sync nötig.")} className="border px-2 disabled:cursor-not-allowed disabled:opacity-50" onClick={()=>decide(mission?"mission":"agent","approve",row)}>Zustimmen</button><button disabled={!row.canDecide} title={row.canDecide ? "" : String(row.decisionHint || "Noch nicht in Firestore-Mirror synchronisiert; erst Inbox-Sync nötig.")} className="border px-2 disabled:cursor-not-allowed disabled:opacity-50" onClick={()=>decide(mission?"mission":"agent","reject",row)}>Ablehnen</button><button disabled={!row.canDecide} title={row.canDecide ? "" : String(row.decisionHint || "Noch nicht in Firestore-Mirror synchronisiert; erst Inbox-Sync nötig.")} className="border px-2 disabled:cursor-not-allowed disabled:opacity-50" onClick={()=>decide(mission?"mission":"agent","revise",row)}>Überarbeiten</button><button disabled={!row.canDecide} title={row.canDecide ? "" : String(row.decisionHint || "Noch nicht in Firestore-Mirror synchronisiert; erst Inbox-Sync nötig.")} className="border px-2 disabled:cursor-not-allowed disabled:opacity-50" onClick={()=>decide(mission?"mission":"agent","block",row)}>Blockieren</button></div></div>;})}</div></section>;
}
