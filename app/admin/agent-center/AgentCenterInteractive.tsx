  ...
  async function runSync(kind: "product" | "local") {
    ...
    try {
      const result = kind === "product"
        ? await beta1AdminClient.syncProductEvolutionFirstRunInbox()
      ...
      if (!typed.accepted) {
        ...
        setSyncStatus(`Sync erfolgreich: erstellt ${typed.created ?? 0}, aktualisiert ${typed.updated ?? 0}, übersprungen ${typed.skipped ?? 0}, gesamt ${typed.syncedCount ?? 0}.`);
        await refreshInbox();
      }
    } catch {
