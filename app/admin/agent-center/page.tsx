...
import { getAgentStatusBucket, getMissionStatusBucket } from "@/lib/admin/agentCenterStatus";
...
const sourceEntries = [
  ...entriesOf(agentCatalog).map((entry) => ({ ...entry, source: "project-register/agent-catalog.json", sourceLabel: "Catalog", canDecide: true })),
  ...entriesOf(approvedAgentBuildBacklog).map((entry) => ({ ...entry, source: "project-register/approved-agent-build-backlog.json", sourceLabel: "Backlog", canDecide: true })),
  ...entriesOf(agentBuildProposals).map((entry) => ({ ...entry, source: "project-register/agent-build-proposals.json", sourceLabel: "Proposal", canDecide: true })),
  ...normalizeFirstRunRows(productEvolutionFirstRunOutput as AnyRow),
];

const adminAgentRows = Array.from(new Map(sourceEntries.map((entry) => { const row = entry as AnyRow; return [String(row.id || row.title || row.name), row]; })).values());
