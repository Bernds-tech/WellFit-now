...
export default function AgentCenterPage() {
  ...
  return <AppShell reward={0} contentClassName="px-0 py-0"><div className="min-h-0 flex-1 overflow-y-auto bg-[linear-gradient(135deg,_#06252d,_#07111f_54%,_#0f172a)] px-5 py-6 pb-24 text-white sm:px-8 lg:px-12"><div className="mx-auto flex max-w-7xl flex-col gap-4"><AgentCenterInteractive firstRunRegisterSnapshot={firstRunRegisterSnapshot} firstRunRegisterSnapshotKeys={firstRunRegisterSnapshotKeys} data={{ agents: adminAgentRows, missions: adminMissionRows, stats: { agent_total: adminAgentRows.length, agent_pending: Number(agentBuckets.pending_approval ?? 0), agent_approved: Number(agentBuckets.approved_ready ?? 0), agent_rejected: Number(agentBuckets.rejected ?? 0), agent_blocked: Number(agentBuckets.blocked ?? 0), agent_in_progress: Number(agentBuckets.in_progress ?? 0), agent_completed: Number(agentBuckets.completed ?? 0), agent_repair_required: Number(agentBuckets.repair_required ?? 0), agent_halted_waiting_owner: Number(agentBuckets.halted_waiting_owner ?? 0), agent_cycle_restart_required: Number(agentBuckets.cycle_restart_required ?? 0), mission_total: adminMissionRows.length, mission_pending: Number(missionBuckets.pending_approval ?? 0), mission_approved: Number(missionBuckets.approved_ready ?? 0), mission_rejected: Number(missionBuckets.rejected ?? 0), mission_blocked: Number(missionBuckets.blocked ?? 0), mission_in_progress: Number(missionBuckets.in_progress ?? 0), mission_completed: Number(missionBuckets.completed ?? 0) } }} /></div></div></AppShell>;
}
