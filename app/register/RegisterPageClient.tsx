...
export default function RegisterPageClient() {
  ...
  const handleRegister = async () => {
    try {
      setIsCreating(true); const userCredential = await createUserWithEmailAndPassword(auth, form.email, form.password); const firebaseUser = userCredential.user; const now = new Date().toISOString(); const displayName = `${form.firstName} ${form.lastName}`.trim(); const targetWeightValue = biometrics.targetWeight ? biometrics.weight : 0;
