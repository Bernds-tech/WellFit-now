...
const buddies: Buddy[] = [
  { id: "flammi", label: "Flammi", file: "flammi.png", description: "Mutiger Drachen-Buddy fÃ¼r deinen WellFit-Start." },
  { id: "luma", label: "Luma", file: "luma.png", description: "Ruhiger Licht-Buddy fÃ¼r Balance und gesunde Routinen." },
  { id: "turt", label: "Turt", file: "turt.png", description: "Geduldiger Buddy fÃ¼r Ausdauer und langfristige Ziele." },
  { id: "king", label: "King", file: "king.png", description: "Power-Buddy fÃ¼r WettkÃ¤mpfe und starke Ziele." },
  { id: "ghost", label: "Ghost", file: "gohst.png", description: "Geheimnisvoller Buddy fÃ¼r stille StÃ¤rke, Fokus und Abenteuer." },
];
...
export default function Step2Biometrics({ language, biometrics, setBiometrics, onNext }: Props) {
  ...
  return (
    <section className="absolute inset-x-8 top-16 bottom-8 overflow-hidden">
      <div className="mb-5 text-center"><div className="text-[10px] font-bold tracking-[0.22em] text-cyan-100/55">{t.biometricsPhase}</div><h1 className="mt-1 text-2xl font-extrabold tracking-tight text-white">{t.setupTitle}</h1></div>
      <div className="grid h-[calc(100%-64px)] grid-cols-[1.15fr_0.95fr] gap-5">
        <div className="min-h-0 overflow-hidden pr-1 pt-9">
        ...
        <aside className="flex min-h-0 flex-col gap-3"><div className="relative flex-1 overflow-hidden rounded-[28px] border border-white/15 bg-white/10 p-5 shadow-[0_10px_30px_rgba(0,0,0,0.12)] backdrop-blur-sm"><div className="relative mb-2 flex items-center justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-cyan-100/70">Buddy-Auswahl</p><p className="text-xs text-white/70">{selectedBuddy.label} | {fitnessText} | BMI {bmi.toFixed(1)} {bmiLabel}</p></div><button type="button" className="rounded-xl bg-black/20 px-4 py-2 text-xs font-bold text-cyan-100 transition hover:bg-black/30">{t.scan}</button></div><div className="relative flex h-[calc(100%-54px)] flex-col items-center justify-center text-center"><div className="relative flex w-full flex-1 items-center justify-center"><button type="button" onClick={() => moveBuddy(-1)} className="absolute left-2 z-10 grid h-12 w-12 place-items-center rounded-full border border-white/15 bg-black/20 text-3xl font-black text-cyan-100 transition hover:bg-black/30" aria-label="Vorheriger Buddy">â€¹</button><div className="grid h-72 w-72 place-items-center rounded-full border border-white/20 bg-cyan-100/15 shadow-[0_0_70px_rgba(103,232,249,0.24)]"><img src={`/buddy/${selectedBuddy.file}`} alt={selectedBuddy.label} className="max-h-[265px] max-w-[265px] object-contain drop-shadow-[0_18px_35px_rgba(0,0,0,0.32)]" /></div><button type="button" onClick={() => moveBuddy(1)} className="absolute right-2 z-10 grid h-12 w-12 place-items-center rounded-full border border-white/15 bg-black/20 text-3xl font-black text-cyan-100 transition hover:bg-black/30" aria-label="NÃ¤chster Buddy">â€º</button></div><div className="mb-4 rounded-2xl border border-white/10 bg-black/15 px-5 py-3"><div className="text-4xl font-black text-cyan-100 drop-shadow-[0_0_14px_rgba(103,232,249,0.25)]">{selectedBuddy.label}</div><p className="mt-1 max-w-[440px] text-sm leading-relaxed text-white/75">{selectedBuddy.description}</p></div></div></div><div className="flex justify-end pb-1"><div className="w-[220px]"><PrimaryButton onClick={onNext}>{t.next}</PrimaryButton></div></div></aside>
      </div>
    </section>
  );
}
