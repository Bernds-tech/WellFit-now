...
export default function Step1Account({ language, form, setForm, onNext }: Props) {
  ...
  const checks = [["length", language === "de" ? "mindestens 8 Zeichen" : "at least 8 characters"], ["uppercase", language === "de" ? "Großbuchstabe" : "uppercase letter"], ["lowercase", language === "de" ? "Kleinbuchstabe" : "lowercase letter"], ["number", language === "de" ? "Zahl" : "number"], ["special", language === "de" ? "Sonderzeichen" : "special character"]] as const;
    ...
    <div className="absolute right-16 top-[8%] z-20 flex h-[760px] w-[520px] flex-col overflow-hidden rounded-[34px] border border-white/20 bg-cyan-500/25 px-7 py-5 shadow-[0_18px_50px_rgba(0,0,0,0.24)] backdrop-blur-md">
      ...
      <div className="relative mt-4 space-y-2.5">
        ...
        <div className="h-[168px]">
          {(form.password ?? "").length > 0 && <div className="h-full rounded-2xl border border-white/20 bg-black/25 px-4 py-3 text-sm text-white shadow-[0_8px_25px_rgba(0,0,0,0.12)] backdrop-blur-sm"><div className="mb-2 flex items-center justify-between"><span className="font-semibold">{language === "de" ? "Passwort-XP" : "Password XP"}</span><span className={`font-black ${passwordStrength.colorClass}`}>{passwordStrength.label}</span></div><div className="h-2.5 w-full overflow-hidden rounded-full bg-white/20"><div className={`h-full rounded-full ${passwordStrength.barClass} transition-all duration-500`} style={{ width: `${Math.max(passwordStrength.score, 1) * 20}%` }} /></div><div className="mt-3 grid grid-cols-2 gap-1.5 text-xs text-white/85">{checks.map(([key, label]) => <div key={key} className="flex items-center gap-2 rounded-lg bg-white/5 px-2 py-1"><span className={passwordStrength.checks[key] ? "text-green-300" : "text-white/45"}>{passwordStrength.checks[key] ? "OK" : "-"}</span><span>{label}</span></div>)}</div></div>}
        </div>
