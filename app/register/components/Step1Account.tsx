...
export default function Step1Account({ language, form, setForm, onNext }: Props) {
  ...
  return (
    <div className="relative z-20 flex w-full flex-col overflow-hidden rounded-[28px] border border-white/20 bg-cyan-500/25 px-4 py-4 shadow-[0_18px_50px_rgba(0,0,0,0.24)] backdrop-blur-md sm:rounded-[34px] sm:px-7 sm:py-5 xl:h-[560px]">
      ...
      <div className="relative mb-4 h-2 overflow-hidden rounded-full bg-white/20"><div className="h-full rounded-full bg-gradient-to-r from-emerald-300 via-cyan-300 to-sky-400 transition-all duration-500" style={{ width: `${setupProgress}%` }} /></div>
      <div className="relative space-y-2.5">
        ...
        <div className="h-[98px] overflow-hidden"><div className="h-full rounded-2xl border border-white/20 bg-black/25 px-4 py-2 text-xs text-white shadow-[0_8px_25px_rgba(0,0,0,0.12)] backdrop-blur-sm"><div className="mb-1.5 flex items-center justify-between"><span className="font-semibold">{language === "de" ? "Passwort-XP" : "Password XP"}</span><span className={`font-black ${passwordStrength.colorClass}`}>{passwordStrength.label}</span></div><div className="h-2 w-full overflow-hidden rounded-full bg-white/20"><div className={`h-full rounded-full ${passwordStrength.barClass} transition-all duration-500`} style={{ width: `${Math.max(passwordStrength.score, 1) * 20}%` }} /></div><div className="mt-2 grid grid-cols-2 gap-1 text-[10px] text-white/85 sm:grid-cols-3">{checks.map(([key, label]) => <div key={key} className="flex items-center gap-1 rounded-md bg-white/5 px-1.5 py-0.5"><span className={passwordStrength.checks[key] ? "text-green-300" : "text-white/45"}>{passwordStrength.checks[key] ? "OK" : "-"}</span><span className="truncate">{label}</span></div>)}</div></div></div>
