...
export default function Step1Account({ language, form, setForm, onNext }: Props) {
  ...
  return (
    <div className="absolute right-16 top-[8%] z-20 w-[520px] overflow-hidden rounded-[34px] border border-white/20 bg-cyan-500/25 px-7 py-5 shadow-[0_18px_50px_rgba(0,0,0,0.24)] backdrop-blur-md">
      ...
      <div className="relative mb-4 flex items-center justify-between gap-4 rounded-2xl border border-white/15 bg-black/15 px-4 py-3">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.28em] text-cyan-100/80">Phase 01</p>
          <p className="mt-1 text-sm font-semibold text-white/90">{language === "de" ? "Account-Erstellung" : "Account creation"}</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="text-right">
            <p className="text-xs text-white/70">{buddyMood}</p>
            <p className="text-lg font-black text-white">{setupProgress}%</p>
          </div>
          <div className="grid h-12 w-12 place-items-center rounded-full bg-white/20 text-2xl shadow-[0_0_20px_rgba(103,232,249,0.35)]">🐉</div>
        </div>
      </div>
      ...
      <div className="relative mt-4 space-y-2.5">
        ...
        {(form.password ?? "").length > 0 && (
          <div className="rounded-2xl border border-white/20 bg-black/25 px-4 py-3 text-sm text-white shadow-[0_8px_25px_rgba(0,0,0,0.12)] backdrop-blur-sm">
            <div className="mb-2 flex items-center justify-between">
              <span className="font-semibold">{language === "de" ? "Passwort-XP" : "Password XP"}</span>
            ...
            <div className="mt-3 grid grid-cols-2 gap-1.5 text-xs text-white/85">
              {checks.map(([key, label]) => (
                <div key={key} className="flex items-center gap-2 rounded-lg bg-white/5 px-2 py-1">
                  <span className={passwordStrength.checks[key] ? "text-green-300" : "text-white/45"}>{passwordStrength.checks[key] ? "✓" : "•"}</span>
                  <span>{label}</span>
                </div>
              ))}
            </div>
      ...
      <label className="relative mt-3 flex items-start gap-3 rounded-2xl border border-white/10 bg-white/10 px-3 py-2.5 text-[0.86rem] leading-snug text-white/90 transition hover:bg-white/15">
        <input type="checkbox" checked={form.accepted ?? true} onChange={(e) => setForm({ ...form, accepted: e.target.checked })} className="mt-1 h-4 w-4 shrink-0 accent-cyan-300" />
        <span>{t.acceptPrefix}<Link href="/datenschutz" className="font-semibold underline">{t.privacyPolicy}</Link>{t.and}<Link href="/agb" className="font-semibold underline">{t.terms}</Link></span>
      </label>
