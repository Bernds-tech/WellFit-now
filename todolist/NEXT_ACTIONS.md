...
- [ ] Naechster Umsetzungsschritt: Implementierungs-PR auf `runtime/beta1-client-read-projections` starten (safe read-only client projections, ohne neue Authority).
- [ ] Danach Implementierungs-PR auf `runtime/beta1-admin-panel-integration` starten (bestehende Admin-Callables anbinden, ohne neue Authority).
