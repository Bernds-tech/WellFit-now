...
- [x] 2026-05-18 follow-up on branch `ci/beta1-emulator-results-followup`: no fake green result; this checkout does not contain `.github/workflows/beta1-emulator-tests.yml`, has no configured Git remote, lacks `gh`, and direct GitHub shell access is blocked by CONNECT tunnel 403, so no real `Beta 1 Emulator Tests` run/log could be evaluated. `beta1:rules`, `beta1:callable`, and Firestore Emulator startup remain unverified; the known Firestore Emulator JAR 403/`ECONNREFUSED 127.0.0.1:8080` blocker remains critical until a real workflow_dispatch or PR run proves otherwise.
- [ ] Next recommended branch after a real run is available: `ci/beta1-emulator-run-log-review` to record exact GitHub Actions run ID, logs, `beta1:rules`/`beta1:callable` execution, Firestore Emulator startup, and whether the JAR download 403 is resolved.
