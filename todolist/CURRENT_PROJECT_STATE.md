...
- Mobile/PWA Device-Testplanung wurde am 2026-05-15 docs/register-only nachgezogen und um eine Manual-Device-Evidence-Vorlage erweitert: bestehende Mobile-Routen, Android Chrome, Samsung Internet, iPhone Safari, Desktop-Responsive-Smoke, QR-/PWA-Install, Kamera-Permission, MediaPipe/Pose/Face, DeviceMotion, WebGL/3D-Flammi, AR-Fallbacks, Screenshot-Smoke, Evidence-Felder, Statuswerte (`pass`/`fail`/`blocked`/`device_test_required`/`review_required`), Severity und Privacy-/Reward-Authority-Grenzen sind in bestehenden Planungsquellen und Registern dokumentiert. Runtime-Mobile-Code, Service Worker, Manifest/Public Assets, Tracking, Datenerfassung, Consent-Flows, Reward-/Mission-Authority, Functions/Rules, Unity/PR #13 und Produktlogik blieben unveraendert.
