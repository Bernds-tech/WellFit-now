...
| `scripts/wellfit-dev-agent/src/cross-reference-maintenance-check.mjs` | aktiv / Code | validiert Cross-Reference Maintenance Register, Pflichtkategorien, Kategorie-Felder, Datei-Existenz, WORK_MAP-/TODO_INDEX-Verweise und Major-Register-Abdeckung | `project-register/cross-reference-maintenance.json` | wird durch Quality Gate ausgefuehrt; bei neuen Registerfamilien Kategorie-Abdeckung pruefen |
| `scripts/wellfit-dev-agent/src/quality-gate.mjs` | aktiv / Code | fuehrt Kontrollkette inklusive Drift-/Gap-Detektoren, Agent-Governance-Control-Check, Research-Recommendation-Check, Adaptive-User-Insight-Check, Firestore-Economy-Rules-Check, Next-Task-Picker und TODO-Status-Sync und Cross-Reference-Maintenance-Check aus und entscheidet PASS/FAIL | Agent-Code | keine KI-Prompt-Pflicht |
...
| `project-register/cross-reference-maintenance.json` | aktiv / Register | maschinenlesbare Cross-Reference-Wartungsmatrix fuer Change Categories, requiredInspectFiles, requiredUpdateTargets, forbiddenAutoUpdates, Human-Review-Regeln und Validierungspflichten | `docs/architecture/WELLFIT_CROSS_REFERENCE_MAINTENANCE.md`, `scripts/wellfit-dev-agent/src/cross-reference-maintenance-check.mjs` | vor/nach Route-, API-, Feature-, Agent-, Roadmap-, Readiness-, Feedback-, Insight-, Research-, Visual-, Dokumentations-, Unity/AR- oder Compliance-Aenderungen lesen |
| `docs/architecture/WELLFIT_CROSS_REFERENCE_MAINTENANCE.md` | aktiv / Agent Governance | human-readable Anleitung fuer die Cross-Reference Maintenance nach jeder Aenderung ohne Parallelarchitektur | `project-register/cross-reference-maintenance.json` | bei Governance-/Register-/TODO-/Script-Aenderungen synchron halten |
| `docs/architecture/WELLFIT_AGENT_SYSTEM_ANALYSIS.md` | aktiv / Agent Governance | aktuelle Analyse des WellFit Agent-/Autopilot-/Governance-Systems, Memory-Dateien, Task-Auswahl, Cooldown-/Loop-Guard, Validation Scripts, Registerfamilien, Protected Areas und Autopilot-Grenzen | `project-register/cross-reference-maintenance.json`, `project-register/agent-autopilot.json` | bei groesseren Agent-Governance-Aenderungen aktualisieren |
