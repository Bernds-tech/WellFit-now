...
const { readGithubRunnerConfig, isProtectedBranchName, githubApiImplementationAvailable, buildGithubRunnerCapability } = require("./agentGithubRunner");
...
function registerAgentAdminRolesAudit(exportsTarget, { db, onCall, HttpsError }) {
  ...
  exportsTarget.prepareAgentGithubRunnerJob = onCall(async (request)=>{ const { actorId, actorRole } = requireRole(request, HttpsError,["owner","agent_supervisor","admin_operator"]); const workerQueueId=requiredString(request.data&&request.data.workerQueueId,"workerQueueId",HttpsError,180); const policyId=requiredString(request.data&&request.data.policyId,"policyId",HttpsError,180); const automationControl = (await getGlobalAutomationControl(db)).data; assertAutomationMayStartNewWork(automationControl, HttpsError, optionalString(request.data&&request.data.taskType,80)); const worker=(await db.collection("agentTaskWorkerQueue").doc(workerQueueId).get()).data()||{}; const policy=(await db.collection("agentTaskAutomationPolicies").doc(policyId).get()).data()||{}; const branchName=optionalString(request.data&&request.data.githubBranchName,180)||optionalString(worker.branchName,180)||`runtime/${workerQueueId}`; if (isProtectedBranchName(branchName)) throw new HttpsError("failed-precondition","direct_main_write_blocked"); const cfg=readGithubRunnerConfig(); const gs=buildGithubRunnerStatus(cfg); const ref=db.collection("agentTaskSupervisedRunnerJobs").doc(); await ref.set({jobId:ref.id,workerQueueId,policyId,runnerStatus:"metadata_only",githubRunnerStatus:gs.status,githubBranchName:branchName,githubPrRef:null,githubPrUrl:null,githubCommitSha:null,requiredChecksSnapshot:policy.requiredChecks||[],checkResultsSnapshot:policy.checkSummary||[],runnerErrorSummary:gs.status==="missing_server_config"?"Serverseitige GitHub-Runner-Config fehlt.":null,realGithubIntegration:gs.realGithubIntegration,canonicalTruthProtectedFiles:CANONICAL_TRUTH_PROTECTED_FILES,allowedFiles:parseStringList(worker.allowedFiles||[]),blockedFiles:parseStringList(worker.blockedFiles||[]),createdBy:actorId,createdByRole:actorRole,createdAt:FieldValue.serverTimestamp(),updatedAt:FieldValue.serverTimestamp()}); await writeAgentAudit(db,{actorId,actorRole,action:"github_runner_prepared",result:gs.status,approvalId:policyId}); return {accepted:true,jobId:ref.id,status:gs.status}; });
  exportsTarget.createAgentGithubBranchMetadata = onCall(async (request)=>{ const { actorId, actorRole } = requireRole(request,HttpsError,["owner","agent_supervisor","admin_operator"]); const jobId=requiredString(request.data&&request.data.jobId,"jobId",HttpsError,180); const branchName=requiredString(request.data&&request.data.githubBranchName,"githubBranchName",HttpsError,180); if (isProtectedBranchName(branchName)) throw new HttpsError("failed-precondition","direct_main_write_blocked"); await db.collection("agentTaskSupervisedRunnerJobs").doc(jobId).set({githubBranchName:branchName,githubRunnerStatus:"branch_metadata_prepared",updatedAt:FieldValue.serverTimestamp()},{merge:true}); await writeAgentAudit(db,{actorId,actorRole,action:"github_runner_branch_prepared",result:"branch_metadata_prepared"}); return {accepted:true,jobId,status:"branch_metadata_prepared"}; });
  exportsTarget.createAgentGithubPullRequest = onCall(async (request)=>{ const { actorId, actorRole } = requireRole(request,HttpsError,["owner","agent_supervisor","admin_operator"]); const jobId=requiredString(request.data&&request.data.jobId,"jobId",HttpsError,180); const ref=db.collection("agentTaskSupervisedRunnerJobs").doc(jobId); const snap=await ref.get(); if(!snap.exists) throw new HttpsError("not-found","Runner Job nicht gefunden."); const capability=buildGithubRunnerCapability(); const status=!capability.hasServerConfig?"missing_server_config":capability.githubApiImplemented?"pr_created":"github_api_not_implemented"; const githubPrRef=capability.githubApiImplemented?optionalString((request.data&&request.data.githubPrRef),220)||null:null; const githubPrUrl=capability.githubApiImplemented?optionalString((request.data&&request.data.githubPrUrl),500)||null:null; await ref.set({githubRunnerStatus:status,githubPrRef,githubPrUrl,updatedAt:FieldValue.serverTimestamp()},{merge:true}); await writeAgentAudit(db,{actorId,actorRole,action:"github_runner_pr_created",result:status}); return {accepted:true,jobId,status}; });
  exportsTarget.recordAgentGithubCheckStatus = onCall(async (request)=>{ const { actorId, actorRole } = requireRole(request,HttpsError,["owner","agent_supervisor","agent_executor_service"]); const jobId=requiredString(request.data&&request.data.jobId,"jobId",HttpsError,180); const checkResultsSnapshot=request.data&&request.data.checkResultsSnapshot||[]; const allRequiredChecksPassed=!!(request.data&&request.data.allRequiredChecksPassed); await db.collection("agentTaskSupervisedRunnerJobs").doc(jobId).set({checkResultsSnapshot,githubRunnerStatus:allRequiredChecksPassed?"checks_passed":"checks_failed",updatedAt:FieldValue.serverTimestamp()},{merge:true}); await writeAgentAudit(db,{actorId,actorRole,action:"github_runner_check_status_recorded",result:allRequiredChecksPassed?"checks_passed":"checks_failed"}); return {accepted:true,jobId}; });
  exportsTarget.refreshAgentGithubCheckStatus = onCall(async (request)=>{ const { actorId, actorRole } = requireRole(request,HttpsError,["owner","agent_supervisor","admin_operator","agent_executor_service"]); const jobId=requiredString(request.data&&request.data.jobId,"jobId",HttpsError,180); await db.collection("agentTaskSupervisedRunnerJobs").doc(jobId).set({githubRunnerStatus:"checks_pending",updatedAt:FieldValue.serverTimestamp()},{merge:true}); await writeAgentAudit(db,{actorId,actorRole,action:"github_runner_check_status_refresh",result:"checks_pending"}); return {accepted:true,jobId,status:"checks_pending"}; });
  exportsTarget.approveAgentGithubAutoMerge = onCall(async (request)=>{ const { actorId, actorRole } = requireRole(request,HttpsError,["owner","agent_supervisor"]); const jobId=requiredString(request.data&&request.data.jobId,"jobId",HttpsError,180); await db.collection("agentTaskSupervisedRunnerJobs").doc(jobId).set({githubRunnerStatus:"auto_merge_metadata_ready",updatedAt:FieldValue.serverTimestamp()},{merge:true}); await writeAgentAudit(db,{actorId,actorRole,action:"github_runner_auto_merge_approved",result:"auto_merge_metadata_ready"}); return {accepted:true,jobId,status:"auto_merge_metadata_ready"}; });
  exportsTarget.executeAgentGithubAutoMergeMetadataOrReal = onCall(async (request)=>{ const { actorId, actorRole } = requireRole(request,HttpsError,["owner","agent_supervisor","agent_executor_service"]); const jobId=requiredString(request.data&&request.data.jobId,"jobId",HttpsError,180); const snap=await db.collection("agentTaskSupervisedRunnerJobs").doc(jobId).get(); const job=snap.data()||{}; if (isProtectedBranchName(job.githubBranchName)) throw new HttpsError("failed-precondition","direct_main_write_blocked"); const policy=(await db.collection("agentTaskAutomationPolicies").doc(job.policyId||"").get()).data()||{}; if (!policy.autoMergeApproved || !policy.allRequiredChecksPassed || (!policy.qualityGatePassed && !policy.qualityGateOverrideApproved)) throw new HttpsError("failed-precondition","auto_merge_gate_failed"); const capability=buildGithubRunnerCapability(); const realGithubApiResult=!!(request.data&&request.data.realGithubApiResult); const status=!capability.hasServerConfig?"missing_server_config":(!capability.githubApiImplemented||!realGithubApiResult)?"github_api_not_implemented":"auto_merged"; await snap.ref.set({githubRunnerStatus:status,updatedAt:FieldValue.serverTimestamp()},{merge:true}); await writeAgentAudit(db,{actorId,actorRole,action:"github_runner_auto_merge_executed",result:status}); return {accepted:true,jobId,status}; });
