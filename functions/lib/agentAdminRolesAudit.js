...
const { readGithubRunnerConfig, isProtectedBranchName } = require("./agentGithubRunner");
...
function registerAgentAdminRolesAudit(exportsTarget, { db, onCall, HttpsError }) {
  ...
  function buildGithubRunnerStatus(config) {
    if (!config.enabled) return { status: "missing_server_config", realGithubIntegration: false };
    return { status: "ready_for_github", realGithubIntegration: true };
  }
