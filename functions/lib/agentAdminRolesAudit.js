...
const conversationIdeaDossierRegister = require("../config/agent-conversation-idea-dossiers.json");
...
function buildBuilderWorkPackagesSnapshotList(packages = [], guard = {}, limit = 50) {
  const safeLimit = Math.max(1, Number(limit || 50));
  const sorted = [...(Array.isArray(packages) ? packages : [])].sort((a, b) => {
    const orderDiff = Number(a.executionOrder || a.sequenceNumber || 0) - Number(b.executionOrder || b.sequenceNumber || 0);
    if (orderDiff !== 0) return orderDiff;
    return String(a.workPackageId || "").localeCompare(String(b.workPackageId || ""));
  });
  const nextUpWorkPackageId = sanitizeTelemetryText(guard && guard.nextUpWorkPackageId, 180);
  const firstPage = sorted.slice(0, safeLimit);
  if (!nextUpWorkPackageId || firstPage.some((item) => String(item.workPackageId || "") === nextUpWorkPackageId)) return firstPage;
  const nextUp = sorted.find((item) => String(item.workPackageId || "") === nextUpWorkPackageId);
  if (!nextUp) return firstPage;
  return [nextUp, ...firstPage.filter((item) => String(item.workPackageId || "") !== nextUpWorkPackageId)].slice(0, safeLimit);
}
...
function registerAgentAdminRolesAudit(exportsTarget, { db, onCall, HttpsError }) {
 ...
 await ref.set({githubRunnerStatus:'files_committed',githubCommitSha:lastSha||null,updatedAt:FieldValue.serverTimestamp()},{merge:true}); await writeAgentAudit(db,{actorId,actorRole,action:'github_runner_files_committed',result:'files_committed'}); return {accepted:true,jobId,status:'files_committed'}; });
  ...
  exportsTarget.getAgentCenterAutopilotSnapshot = onCall(async (request) => {
    ...
    const conversationIdeaDossierMap = new Map();
    for (const seed of getConversationIdeaSeedDossiers()) {
      const normalizedSeed = normalizeConversationIdeaDossier(seed, { dossierId: seed.dossierId || seed.id, seedDossierId: seed.dossierId || seed.id, source: "seed" });
      if (!isEmptyTechnicalConversationIdeaDossier(normalizedSeed)) conversationIdeaDossierMap.set(normalizedSeed.dossierId, normalizedSeed);
    }
    for (const item of conversationIdeaRaw) {
      const normalizedServer = normalizeConversationIdeaDossier(item, { dossierId: item.dossierId || item.id, serverDossierId: item.id || item.dossierId, source: "server" });
      if (isEmptyTechnicalConversationIdeaDossier(normalizedServer)) continue;
      const seed = conversationIdeaDossierMap.get(normalizedServer.dossierId);
      conversationIdeaDossierMap.set(normalizedServer.dossierId, seed ? { ...seed, ...normalizedServer, title: normalizedServer.title || seed.title, shortSummary: normalizedServer.shortSummary || seed.shortSummary, plainLanguageSummary: normalizedServer.plainLanguageSummary || seed.plainLanguageSummary } : normalizedServer);
    }
    const conversationIdeaDossiers = Array.from(conversationIdeaDossierMap.values());
    ...
    const snapshot = {
      ...
      conversationIdeaDossierCounts: { ...getStatusCountsByBucket(conversationIdeaDossiers, (item) => String(item.status || "pending_approval"), ["pending_approval", "approved", "rejected", "revision_requested", "blocked"]), total: conversationIdeaDossiers.length, serverTotal: conversationIdeaTotal, seedTotal: getConversationIdeaSeedDossiers().length },
      ...
      builderWorkPackages: buildBuilderWorkPackagesSnapshotList(builderPackages, builderGuard, 50),
