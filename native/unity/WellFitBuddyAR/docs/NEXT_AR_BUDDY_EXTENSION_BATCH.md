...
Status: in progress after successful Android AR smoke test.

## Goal

Prepare the next larger test batch before another Android build.
...
1. Buddy idle wobble.
2. Buddy look-at-camera helper.
3. Tap target visual marker.
4. Surface hit quality label.
5. Plane missing hint text.
6. Movement policy helper.
7. Height difference label.
8. Max distance label.
9. Duplicate return request guard.
10. Working scene notes.
11. Dev scene notes.
12. Security event boundary note.
13. Later re-anchor note.
14. Later occlusion note.
15. Later companion radius integration.

## Hard boundary

Unity remains visual only. No rewards, XP, points, token logic, mission completion, leaderboard, jackpot, burn, or anti-cheat authority in Unity.
